...
export default function Home(){
  ...
  async function ownerDeleteResolvedTickets(){
    if(!isAdmin||!confirm('Excluir todos os chamados resolvidos?'))return;
    const ids=ownerTickets.filter(t=>t.status==='resolvido').map(t=>t.id); if(!ids.length)return;
    await supabase.from('cidarank_support_messages').delete().in('ticket_id',ids);
    const {error}=await supabase.from('daquitop_support_tickets').delete().in('id',ids); if(error){alert(error.message);return;} await loadOwnerData();
  }
  ...
  async function explore(){
    ...
    const [{data:p},{data:b},{data:raw},{data:votes}]=await Promise.all([
      ...
      supabase.from('posts').select('*').eq('is_hidden',false).eq('city_key',cityKey(exploreCity)).eq('state_key',stateKey(exploreState)).order('created_at',{ascending:false}).limit(30),
