...
export default function Home(){
  ...
  const [businessFollowersCount,setBusinessFollowersCount]=useState(0);
  const [businessFollowing,setBusinessFollowing]=useState(false);
  ...
  const [messageMediaFile,setMessageMediaFile]=useState(null);
  const [messageMediaPreview,setMessageMediaPreview]=useState('');
  const [messageMediaType,setMessageMediaType]=useState('');
  const [messageMediaUploading,setMessageMediaUploading]=useState(false);
  ...
  const [scopeCompetitionSettings,setScopeCompetitionSettings]=useState([]);
  const [ownerStateCompetitionEnd,setOwnerStateCompetitionEnd]=useState('');
  const [ownerBrazilCompetitionEnd,setOwnerBrazilCompetitionEnd]=useState('');
  const [launchResetText,setLaunchResetText]=useState('');
  const [launchResetBusy,setLaunchResetBusy]=useState(false);

  const [search,setSearch]=useState('');

  useEffect(()=>{
    if(!profile || !['state','brazil'].includes(popularView)) return;
    loadScopeCompetitionRank(popularView);
  },[popularView,profile?.state,scopeCompetitionSettings]);
  ...
  async function handleExploreState(e){
    ...
    const item=states.find(x=>String(x.id)===String(id));
    const uf=item?.sigla||'';
    setExploreStateId(id); setExploreState(uf); setExploreCity(''); setExploreCities([]);
    if(!id)return;
    setExploreBusy(true);
    try{
      await loadPublicExpansionData();
      const {data:openRows,error}=await supabase.from('cidarank_city_expansion').select('state,city,is_open').eq('is_open',true);
      if(error)throw error;
      const rows=(openRows||[]).filter(r=>stateKey(r.state)===stateKey(uf));
      setExploreCities(rows.map((r,i)=>({id:`explore-${uf}-${i}`,nome:r.city})).sort((a,b)=>a.nome.localeCompare(b.nome,'pt-BR')));
    }catch{
      setExploreCities([]);
  ...
  async function loadScopeCompetitionSettings(){
    const {data,error}=await supabase.from('cidarank_scope_competitions').select('*').order('scope').order('scope_key');
    if(error){
      // A V6.22 precisa do SQL novo; manter o restante do site funcional se ainda não foi executado.
      console.warn('Competições Estado/Brasil ainda não instaladas:',error.message);
      setScopeCompetitionSettings([]);
      return;
    }
    setScopeCompetitionSettings(data||[]);
  }

  function scopeCompetitionSetting(scope,uf=profile?.state){
    const key=scope==='brazil'?'brasil':stateKey(uf||'');
    return (scopeCompetitionSettings||[]).find(x=>x.scope===scope&&stateKey(x.scope_key)===stateKey(key))||null;
  }

  function scopeCompetitionEnabled(scope,uf=profile?.state){
    return Boolean(scopeCompetitionSetting(scope,uf)?.enabled);
  }

  function scopeCompetitionOpen(scope,uf=profile?.state){
    const setting=scopeCompetitionSetting(scope,uf);
    return Boolean(setting?.enabled) && (!setting?.ends_at || new Date(setting.ends_at).getTime()>clockNow);
  }

  async function loadScopeCompetitionRank(scope){
    if(!profile || !['state','brazil'].includes(scope))return;
    if(!scopeCompetitionEnabled(scope,profile.state)){setScopeRanking([]);return;}
    setScopeRankingBusy(true);
    const {data,error}=await supabase.rpc('cidarank_scope_ranking',{
      p_scope:scope,
      p_scope_key:scope==='state'?stateKey(profile.state):'brasil',
      p_year:new Date().getFullYear()
    });
    if(error){
      console.error('Erro ranking estadual/nacional:',error);
      setScopeRanking([]);
      setNotice('Não foi possível carregar a competição estadual/nacional agora.');
    }else{
      setScopeRanking((Array.isArray(data)?data:[]).map(x=>({...x,id:x.candidate_id,votes:Number(x.votes||0),followers_count:Number(x.followers_count||0)})));
    }
    setScopeRankingBusy(false);
  }

  async function castScopeVote(person,scope){
    if(!person?.id)return;
    const {error}=await supabase.rpc('cidarank_cast_scope_vote',{p_scope:scope,p_candidate_id:person.id});
    if(error){alert(error.message);return;}
    await loadScopeCompetitionRank(scope);
    setNotice(scope==='state'?'Voto estadual confirmado. Este voto é definitivo nesta edição.':'Voto Brasil confirmado. Este voto é definitivo nesta edição.');
  }

  async function ownerSetScopeCompetition(scope,enabled){
    if(!isOwner)return;
    const targetState=scope==='state'?expansionState:'';
    if(scope==='state'&&!targetState){alert('Escolha um estado antes de liberar a competição estadual.');return;}
    const rawEnd=scope==='state'?ownerStateCompetitionEnd:ownerBrazilCompetitionEnd;
    const endIso=rawEnd?new Date(rawEnd).toISOString():null;
    setExpansionBusy(true);
    const {data,error}=await supabase.rpc('cidarank_set_scope_competition',{
      p_scope:scope,
      p_scope_key:scope==='state'?stateKey(targetState):'brasil',
      p_enabled:enabled,
      p_ends_at:endIso
    });
    setExpansionBusy(false);
    if(error){alert(error.message);return;}
    await loadScopeCompetitionSettings();
    if(scope===popularView)await loadScopeCompetitionRank(scope);
    await logAdminAction(enabled?`${scope}_competition_enabled`:`${scope}_competition_paused`,'competition',scope,data||{});
    setNotice(enabled?(scope==='state'?`Competição Estadual de ${targetState} liberada e sincronizada com Ranking + Tabela.`:'Competição Brasil liberada e sincronizada com Ranking + Tabela.'):(scope==='state'?`Competição Estadual de ${targetState} pausada.`:'Competição Brasil pausada.'));
  }
  ...
  async function handleAuth(e){
    ...
    const {data,error}=await supabase.auth.signUp({email:form.email.trim(),password:form.password,options:{data:{
      ...
      cidarank_terms_accepted:true,cidarank_terms_version:'1.0',cidarank_privacy_version:'1.0',cidarank_terms_accepted_at:new Date().toISOString()
    }}});
    setAuthBusy(false);
  ...
  async function uploadChatMedia(file){
    ...
    if(!isImage&&!isVideo)throw new Error('Envie uma foto ou vídeo válido.');
    const max=isVideo?20*1024*1024:8*1024*1024;
    if(file.size>max)throw new Error(isVideo?'O vídeo deve ter no máximo 20 MB.':'A imagem deve ter no máximo 8 MB.');
    const path=`${session.user.id}/direct-${Date.now()}-${safeName(file.name)}`;
    const {error}=await supabase.storage.from('daquitop-media').upload(path,file,{upsert:false,contentType:file.type});
    if(error)throw error;
    const url=supabase.storage.from('daquitop-media').getPublicUrl(path).data.publicUrl;
    return {url,type:isVideo?'video':'image'};
  }

  function chooseMessageMedia(file){
    if(!file)return;
    const isImage=file.type.startsWith('image/');
    const isVideo=file.type.startsWith('video/');
    if(!isImage&&!isVideo){alert('Escolha uma foto ou vídeo.');return;}
    if(messageMediaPreview)URL.revokeObjectURL(messageMediaPreview);
    setMessageMediaFile(file);
    setMessageMediaType(isVideo?'video':'image');
    setMessageMediaPreview(URL.createObjectURL(file));
  }

  function clearMessageMedia(){
    if(messageMediaPreview)URL.revokeObjectURL(messageMediaPreview);
    setMessageMediaFile(null);setMessageMediaPreview('');setMessageMediaType('');
  ...
  async function loadBusiness(id){
    ...
    const [{data:r},{data:bp},{data:bf}]=await Promise.all([
      ...
      supabase.from('cidarank_business_follows').select('user_id').eq('business_id',id)
    ]);
    const followers=bf||[];
    setBusinessFollowersCount(followers.length);
    setBusinessFollowing(Boolean(session?.user?.id&&followers.some(x=>x.user_id===session.user.id)));
  ...
  async function toggleBusinessFollow(){
    if(!selectedBusiness||!session?.user?.id||selectedBusiness.owner_id===session.user.id)return;
    if(businessFollowing){
      const {error}=await supabase.from('cidarank_business_follows').delete().eq('business_id',selectedBusiness.id).eq('user_id',session.user.id);
      if(error){alert(error.message);return;}
    }else{
      const {error}=await supabase.from('cidarank_business_follows').upsert({business_id:selectedBusiness.id,user_id:session.user.id},{onConflict:'business_id,user_id'});
      if(error){alert(error.message);return;}
    }
  ...
  async function searchMentionPeople(value){
    ...
    if(!error)setMentionResults(data||[]);
    setMentionBusy(false);
  }

  function handleComposerTextChange(value,setText){
    setText(value);
    const match=String(value||'').match(/(?:^|\s)@([a-zA-Z0-9._-]*)$/);
    if(match){
      setMentionPickerOpen(true);
      searchMentionPeople(match[1]);
  ...
  function addMentionToText(person,setText){
    ...
    setText(v=>{
      ...
      const tag=`@${username}`;
      const partial=/(^|\s)@[a-zA-Z0-9._-]*$/;
      if(partial.test(current))return current.replace(partial,(m,prefix)=>`${prefix}${tag} `);
  ...
  async function sendMessage(e){
    ...
    if((!content&&!messageMediaFile)||!selectedChat)return;
    setMessageMediaUploading(true);
    try{
      let media_url=null,media_type=null;
      if(messageMediaFile){const uploaded=await uploadChatMedia(messageMediaFile);media_url=uploaded.url;media_type=uploaded.type;}
      const {error}=await supabase.from('daquitop_messages').insert({sender_id:session.user.id,recipient_id:selectedChat.id,content,media_url,media_type});
      if(error)throw error;
      setMessageText('');clearMessageMedia();setMessageEmojiOpen(false);
      await loadMessages(session.user.id);
    }catch(err){alert(`Não foi possível enviar: ${err.message}`)}
    setMessageMediaUploading(false);
  }
  ...
  function periodCountdown(type){
    const now=new Date(clockNow), end=periodEnd(type);
  ...
  function periodEndLabel(type){
    const end=periodEnd(type);
    return end.toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
  }
  function countdownTo(iso){
    if(!iso)return 'sem data definida';
    const diff=Math.max(0,new Date(iso).getTime()-clockNow);
    const days=Math.floor(diff/86400000),hours=Math.floor((diff%86400000)/3600000),mins=Math.floor((diff%3600000)/60000);
    return `${days}d ${hours}h ${mins}min`;
  }
  function scopeEndLabel(scope,uf=profile?.state){
    const setting=scopeCompetitionSetting(scope,uf);
    if(!setting?.ends_at)return 'Data definida pelo Dono';
    return new Date(setting.ends_at).toLocaleString('pt-BR',{day:'2-digit',month:'2-digit',year:'numeric',hour:'2-digit',minute:'2-digit'});
  }
  ...
  function renderTextWithMentions(text){
    ...
    const parts=raw.split(/(@[A-Za-z0-9._]+)/g);
    const pool=[...(people||[]),...(exploreData?.people||[]),...Object.values(postAuthors||{}),...Object.values(profilePostAuthors||{}),...Object.values(messagePeople||{})];
    return parts.map((part,i)=>{
      if(!part.startsWith('@'))return <span key={`txt-${i}`}>{part}</span>;
      const username=cleanUsername(part.slice(1));
      const person=pool.find(x=>cleanUsername(x?.username)===username);
      return person?<button key={`mention-${i}`} type="button" className="inlineMention" onClick={()=>openProfile(person)}>{part}</button>:<span key={`mention-${i}`} className="inlineMention plainMention">{part}</span>;
    });
  }
  ...
  function PostCard({post,authors=postAuthors,bizMap=postBusinesses,interactive=true,officialFeed=false}){
    ...
    return <article className="post socialPost" key={post.id}>
      ...
      {post.content&&<p>{renderTextWithMentions(post.content)}</p>}{post.image_url&&<img className="postImage" src={post.image_url} alt="Publicação"/>}
  ...
  function RankingCityScreen(){
    ...
    const isScope=['state','brazil'].includes(popularView);
    const stateEnabled=scopeCompetitionEnabled('state',profile?.state);
    const brazilEnabled=scopeCompetitionEnabled('brazil');
    const rank=isScope?scopeRanking:(rankType?competitionRanking(rankType):popularView==='city'?popularRank:[]);
    const currentVote=isScope?(rank.find(x=>x.voted_by_me)?.id||null):(rankType==='week'?myVote('week'):null);
    ...
    const metric=p=>popularView==='city'?`${p.followers_count||0} seguidores`:isScope?`${p.votes||0} voto(s)`:rankType==='week'?`${p.votes||0} voto(s)`:`${p.points||0} pontos`;
    const status=isScope?{label:scopeCompetitionOpen(popularView,profile?.state)?(popularView==='state'?'VOTAÇÃO ESTADUAL ABERTA':'VOTAÇÃO BRASIL ABERTA'):'VOTAÇÃO ENCERRADA',countdown:countdownTo(scopeCompetitionSetting(popularView,profile?.state)?.ends_at),icon:popularView==='state'?'🗺️':'🇧🇷'}:(rankType?rankStatus(rankType):{label:'POPULARIDADE PERMANENTE',countdown:'Não reinicia',icon:'👑'});
    const title=popularView==='city'?'Mais Popular da Cidade':popularView==='state'?`Ranking Estadual — ${profile?.state||''}`:popularView==='brazil'?'Ranking Brasil':rankType==='week'?'Ranking Semanal':rankType==='month'?'Ranking Mensal':'Ranking Anual';
    const scopeLabel=popularView==='state'?`Estado ${profile?.state||''}`:popularView==='brazil'?'Brasil inteiro':cityLabel;
    return <><PageHeader title="Ranking" subtitle={`Acompanhe posições locais, estaduais e nacionais do CIDARANK.`}/>
      <div className="panel rankingChooser"><div className="segmented wrap">
        <button className={popularView==='city'?'active':''} onClick={()=>setPopularView('city')}>Popular da cidade</button><button className={popularView==='week'?'active':''} onClick={()=>setPopularView('week')}>Semanal</button><button className={popularView==='month'?'active':''} onClick={()=>setPopularView('month')}>Mensal</button><button className={popularView==='year'?'active':''} onClick={()=>setPopularView('year')}>Anual</button><button className={stateEnabled&&popularView==='state'?'active':stateEnabled?'':'lockedFuture'} disabled={!stateEnabled} onClick={()=>stateEnabled&&setPopularView('state')}>🗺️ Estado {stateEnabled?'AO VIVO':'• EM BREVE'}</button><button className={brazilEnabled&&popularView==='brazil'?'active':brazilEnabled?'':'lockedFuture'} disabled={!brazilEnabled} onClick={()=>brazilEnabled&&setPopularView('brazil')}>🇧🇷 Brasil {brazilEnabled?'AO VIVO':'• EM BREVE'}</button><button className={popularView==='business'?'active':''} onClick={()=>setPopularView('business')}>Estabelecimentos</button>
      </div></div>
      <div className="panel rankHow"><h2>Como funciona?</h2><p><b>Cidade:</b> Semanal → Mensal → Anual. Ao fim do ciclo, os <b>10 melhores do Anual de cada cidade</b> podem representar sua cidade no Estadual. <b>Estado:</b> pessoas do mesmo estado votam entre os classificados das cidades; os <b>10 melhores do Estado</b> avançam ao Brasil. <b>Brasil:</b> qualquer usuário do país pode votar entre os classificados estaduais.</p><div className="rankFlow"><span>🗳️ SEMANAL</span><b>→</b><span>🏆 MENSAL</span><b>→</b><span>👑 TOP 10 DA CIDADE</span><b>→</b><span className={stateEnabled?'scopeLiveMini':'futureMini'}>🗺️ ESTADO {stateEnabled?'AO VIVO':'EM BREVE'}</span><b>→</b><span className={brazilEnabled?'scopeLiveMini':'futureMini'}>🇧🇷 BRASIL {brazilEnabled?'AO VIVO':'EM BREVE'}</span></div></div>
      {popularView!=='business'&&<><section className="panel mainRankStage"><div className="panelTitle"><div><span className="eyebrow">AO VIVO • {scopeLabel}</span><h2>{title}</h2><p>TOP 3 em destaque e classificação numerada logo abaixo.</p></div><span className="livePill">● ATUALIZAÇÃO AUTOMÁTICA</span></div><div className="rankStatusBar"><span>{status.icon}</span><div><b>{status.label}</b><small>{isScope?<>Encerra em <strong>{scopeEndLabel(popularView)}</strong> • faltam <strong>{status.countdown}</strong></>:rankType?<>Encerra em <strong>{periodEndLabel(rankType)}</strong> • faltam <strong>{status.countdown}</strong></>:<>Status: <strong>{status.countdown}</strong></>}</small></div></div><div className="liveClassificationCard"><div className="liveClassificationHead"><div><span>📊 TABELA DE CLASSIFICAÇÃO</span><b>{title} • {scopeLabel}</b></div><em>● AO VIVO</em></div><div className="liveClassificationRows">{rank.slice(0,10).map((p,i)=><button key={`live-${p.id}`} className={`liveClassificationRow ${p.id===session.user.id?'mine':''}`} onClick={()=>openProfile(p)}><strong>{i===0?'🥇':i===1?'🥈':i===2?'🥉':`#${i+1}`}</strong><Avatar profile={p} size="xs"/><span><b>{p.full_name||p.username}</b><small>@{p.username}</small></span><em>{metric(p)}</em></button>)}{!rank.length&&<div className="liveClassificationEmpty">Ainda não há classificados. A tabela será preenchida automaticamente.</div>}</div><div className="liveMyPosition">📍 <span>SUA POSIÇÃO AGORA</span><b>{myPos>=0?`#${myPos+1}`:'—'}</b><small>{myPos>=0?`${metric(rank[myPos])} • ${rank.length} classificados`:'Você ainda não entrou nesta classificação.'}</small></div></div><div className="floatingPodium rankPagePodium podiumWithNationalBg" style={{'--podium-bg':`url(${bannerForRank(popularView)})`}}>{scopeRankingBusy&&isScope?<div className="rankEmptyState podiumEmptyOnBanner"><span>⏳</span><b>Carregando ranking...</b></div>:top3.length?order.map(p=>{const pos=rank.findIndex(x=>x.id===p.id)+1;return <button className={`floatingWinner place${pos} animatedRankPerson`} key={p.id} onClick={()=>openProfile(p)}><span className="floatingPlace">#{pos}</span><span className="floatingCrown">{pos===1?'♛':pos===2?'◆':'▲'}</span><span className="rankPhotoSquare"><Avatar profile={p} name={p.full_name} size={pos===1?'lg':'md'}/></span><div className="winnerIdentity"><b>{p.full_name||p.username}</b><Seal type={sealTypeFor(p)} color={p.seal_color}/></div><small>@{p.username}</small><strong>{metric(p)}</strong><span className="podiumBase">{pos}º</span></button>}):<div className="rankEmptyState podiumEmptyOnBanner"><span>🏆</span><b>Ainda não há classificados</b><small>O banner deste ranking já está ativo. Assim que houver votos, pontos ou seguidores válidos, o TOP 3 aparece automaticamente.</small></div>}</div>{myPos>=0&&<div className="myRankSpot"><span>📍 SUA POSIÇÃO</span><b>#{myPos+1}</b><small>de {rank.length} classificados • {metric(rank[myPos])}</small></div>}{myPos<0&&!scopeRankingBusy&&<div className="myRankSpot"><span>📍 SUA POSIÇÃO</span><b>—</b><small>Ainda não classificado neste ranking.</small></div>}</section>
      ...
      {isScope&&<div className="panel weeklyVotePanel scopeVotePanel"><div className="panelTitle"><div><h2>{popularView==='state'?'🗺️ Votação Estadual':'🇧🇷 Votação Brasil'}</h2><p>{popularView==='state'?`Somente usuários de ${profile?.state||'seu estado'} podem votar. Os participantes são os TOP 10 anuais de cada cidade classificada.`:'Qualquer usuário do Brasil pode votar. Os participantes são os TOP 10 classificados de cada Estado.'}</p></div><b>{rank.length} classificados</b></div><div className="listTable">{rank.map((p,i)=><div className={`listRow ${p.voted_by_me?'myRankRow':''}`} key={`scope-vote-${p.id}`}><b className="rankNum">#{i+1}</b><Avatar profile={p}/><div className="grow"><button className="linkName" onClick={()=>openProfile(p)}>{p.full_name||p.username}</button><small>@{p.username} • {p.city||'Cidade'}{p.state?` - ${p.state}`:''}</small></div><strong>{p.votes||0} voto(s)</strong><button className={p.voted_by_me?'voteBtn active':'voteBtn'} disabled={p.id===session.user.id||!scopeCompetitionOpen(popularView,profile?.state)} onClick={()=>castScopeVote(p,popularView)}>{p.id===session.user.id?'VOCÊ':!scopeCompetitionOpen(popularView,profile?.state)?'ENCERRADO':p.voted_by_me?'✓ SEU VOTO':'VOTAR'}</button></div>)}{!rank.length&&!scopeRankingBusy&&<p className="muted">Ainda não há classificados nesta fase. Ao liberar a competição, o sistema traz automaticamente os classificados elegíveis.</p>}</div></div>}
  ...
  function ClassificationTableScreen(){
    const stateEnabled=scopeCompetitionEnabled('state',profile?.state);
    const brazilEnabled=scopeCompetitionEnabled('brazil');
    const view=['city','week','month','year','state','brazil'].includes(popularView)?popularView:'week';
    const popularRank=[...people].filter(p=>!isInstitutionalOwner(p)&&(p.followers_count||0)>0).sort((a,b)=>(b.followers_count||0)-(a.followers_count||0));
    const isScope=['state','brazil'].includes(view);
    const rank=view==='city'?popularRank:isScope?scopeRanking:competitionRanking(view);
    const myPos=rank.findIndex(p=>p.id===session.user.id);
    const status=view==='city'?null:isScope?{countdown:countdownTo(scopeCompetitionSetting(view,profile?.state)?.ends_at)}:rankStatus(view);
    const labels={city:['Popular da Cidade','⭐','seguidores'],week:['Semanal','🗳️','voto(s)'],month:['Mensal','🏆','pontos'],year:['Anual','👑','pontos'],state:['Estadual','🗺️','voto(s)'],brazil:['Brasil','🇧🇷','voto(s)']};
    const [label,icon,unit]=labels[view];
    const metric=p=>view==='city'?`${p?.followers_count||0} ${unit}`:(view==='week'||isScope)?`${p?.votes||0} ${unit}`:`${p?.points||0} ${unit}`;
    const tableScope=view==='state'?`Estado ${profile?.state||''}`:view==='brazil'?'Brasil inteiro':cityLabel;
    return <>
      <PageHeader title="Tabela de Classificação" subtitle={`Classificação automática e sincronizada com o Ranking • ${tableScope}.`}/>
      <section className="panel classificationHub">
        <div className="classificationTabs">
          <button className={view==='city'?'active':''} onClick={()=>setPopularView('city')}>⭐ Popular da Cidade</button>
          {['week','month','year'].map(k=><button key={k} className={view===k?'active':''} onClick={()=>setPopularView(k)}>{labels[k][1]} {labels[k][0]}</button>)}
          <button className={stateEnabled&&view==='state'?'active':stateEnabled?'':'locked'} disabled={!stateEnabled} onClick={()=>stateEnabled&&setPopularView('state')}>🗺️ Estado <small>{stateEnabled?'AO VIVO':'EM BREVE'}</small></button>
          <button className={brazilEnabled&&view==='brazil'?'active':brazilEnabled?'':'locked'} disabled={!brazilEnabled} onClick={()=>brazilEnabled&&setPopularView('brazil')}>🇧🇷 Brasil <small>{brazilEnabled?'AO VIVO':'EM BREVE'}</small></button>
        </div>
        <div className="classificationLiveHead"><div><span>● {view==='city'?'ATUALIZAÇÃO CONTÍNUA':'AO VIVO'}</span><h2>{icon} Classificação {label}</h2><p>{view==='city'?'Ordenada automaticamente pelos seguidores válidos da cidade.':isScope?'Mesmos classificados e votos exibidos no Ranking desta fase.':'Atualização automática conforme votos e pontos válidos.'}</p></div><div className="classificationClock"><small>{view==='city'?'STATUS':'ENCERRA EM'}</small><b>{view==='city'?'NÃO REINICIA':isScope?scopeEndLabel(view):periodEndLabel(view)}</b><em>{view==='city'?'Popularidade permanente':`Faltam ${status?.countdown||'—'}`}</em></div></div>
        {isScope&&<div className="classificationQualification"><b>{view==='state'?'🗺️ FASE ESTADUAL':'🇧🇷 FASE NACIONAL'}</b><span>{view==='state'?'Entram os 10 melhores do Anual de cada cidade do Estado.':'Entram os 10 melhores classificados de cada Estado.'}</span></div>}
        <div className="classificationTable">
          <div className="classificationHeader"><b>POS.</b><b>PESSOA</b><b>{view==='city'?'SEGUIDORES':'VOTOS / PONTOS'}</b></div>
          {rank.map((person,i)=><button key={person.id} className={`classificationRow ${person.id===session.user.id?'mine':''}`} onClick={()=>openProfile(person)}><strong>{i===0?'🥇 1º':i===1?'🥈 2º':i===2?'🥉 3º':`#${i+1}`}</strong><span className="classificationIdentity"><Avatar profile={person} size="sm"/><span><b>{person.full_name||person.username}</b><small>@{person.username}{isScope&&person.city?` • ${person.city} - ${person.state||''}`:''}</small></span></span><em>{metric(person)}</em></button>)}
        ...
        <div className="classificationMyPosition"><div><span>📍 SUA POSIÇÃO AGORA</span><b>{myPos>=0?`#${myPos+1}`:'—'}</b></div><div>{myPos>=0?<><strong>{metric(rank[myPos])}</strong><small>{myPos>0?`Você está em #${myPos+1}. A classificação muda automaticamente em tempo real.`:'Você está liderando esta classificação.'}</small></>:<><strong>Ainda não classificado</strong><small>{isScope?'Você só aparece aqui se estiver classificado para esta fase.':'Quando você receber votos, pontos ou seguidores válidos, sua posição aparece aqui automaticamente.'}</small></>}</div></div>
      </section>
    </>
  }
  ...
  function BusinessScreen(){
    ...
    if(selectedBusiness){
      ...
      const avg=Number(selectedBusiness.rating_average||0);
      const reviews=Number(selectedBusiness.reviews_count||businessReviews.length||0);
      return <><button className="backBtn" onClick={()=>setSelectedBusiness(null)}>← Voltar para estabelecimentos</button>
        <section className="businessProfileV2">
          <div className="businessHeroV2">{selectedBusiness.cover_url?<img className="businessHeroCover" src={selectedBusiness.cover_url} alt={`Capa de ${selectedBusiness.name}`}/>:<div className="businessHeroCoverEmpty">CIDARANK • COMÉRCIO LOCAL</div>}<div className="businessHeroShade"/><div className="businessHeroIdentity"><div className="businessLogoFrame"><Avatar profile={{avatar_url:selectedBusiness.logo_url}} name={selectedBusiness.name} size="xl"/></div><div className="grow"><span className="businessCategoryPill">🏪 {selectedBusiness.category||'Estabelecimento local'}</span><h1>{selectedBusiness.name} <Seal type="business"/></h1><p>@{selectedBusiness.username||'comercio'} • 📍 {selectedBusiness.city} - {selectedBusiness.state}</p><div className="businessRatingLine"><b>★ {avg.toFixed(1)}</b><span>{ratingStars(avg)}</span><small>{reviews} {reviews===1?'avaliação':'avaliações'}</small></div></div><div className="businessHeroActions">{!mine&&<button className={businessFollowing?'secondaryBtn':'primaryBtn'} onClick={toggleBusinessFollow}>{businessFollowing?'✓ Seguindo':'＋ Seguir'}</button>}{selectedBusiness.owner_id&&selectedBusiness.owner_id!==session.user.id&&<button className="secondaryBtn" onClick={async()=>{const {data}=await supabase.from('profiles').select('*').eq('id',selectedBusiness.owner_id).maybeSingle();if(data)openChat(data)}}>💬 Mensagem</button>}</div></div></div>
          <div className="businessStatsV2"><div><b>{businessFollowersCount}</b><span>Seguidores</span></div><div><b>★ {avg.toFixed(1)}</b><span>Nota média</span></div><div><b>{reviews}</b><span>Avaliações</span></div><div><b>{businessPosts.length}</b><span>Publicações</span></div></div>
        </section>
        <div className="twoCol businessTwoCol"><div>
          <div className="panel businessAbout"><div className="panelTitle"><div><span className="eyebrow">PERFIL COMERCIAL</span><h2>Sobre o estabelecimento</h2></div></div><p>{selectedBusiness.description||'Este estabelecimento ainda não adicionou uma descrição.'}</p><div className="businessContactGrid"><span>📍 <b>Endereço</b><small>{selectedBusiness.address||`${selectedBusiness.city} - ${selectedBusiness.state}`}</small></span>{selectedBusiness.whatsapp&&<span>📱 <b>WhatsApp</b><small>{selectedBusiness.whatsapp}</small></span>}{selectedBusiness.website&&<span>🌐 <b>Site</b><small>{selectedBusiness.website}</small></span>}</div></div>
          ...
          <div className="panel"><div className="panelTitle"><div><h2>📰 Novidades do estabelecimento</h2><p>Publicações, produtos, serviços e avisos.</p></div></div>{businessPosts.length?<div className="feed">{businessPosts.map(p=>PostCard({post:p,authors:businessPostAuthors,bizMap:{[selectedBusiness.id]:selectedBusiness},interactive:true}))}</div>:<p className="muted">Este estabelecimento ainda não publicou nada.</p>}</div>
        </div><div>
          <div className="panel businessReviewPanel"><div className="reviewScoreHero"><div><strong>{avg.toFixed(1)}</strong><span>{ratingStars(avg)}</span><small>{reviews} {reviews===1?'avaliação':'avaliações'} verificadas no CIDARANK</small></div></div>{!mine&&<form className="stackForm premiumReviewForm" onSubmit={submitReview}><h3>⭐ Avalie este estabelecimento</h3><p className="muted">Sua avaliação é única. Se avaliar novamente, sua nota anterior será atualizada.</p><label>Sua nota<select value={reviewForm.rating} onChange={e=>setReviewForm({...reviewForm,rating:e.target.value})}><option value="5">★★★★★ 5 - Excelente</option><option value="4">★★★★☆ 4 - Muito bom</option><option value="3">★★★☆☆ 3 - Bom</option><option value="2">★★☆☆☆ 2 - Regular</option><option value="1">★☆☆☆☆ 1 - Ruim</option></select></label><label>Comentário<textarea value={reviewForm.comment} onChange={e=>setReviewForm({...reviewForm,comment:e.target.value})} placeholder="Conte sua experiência"/></label><button className="primaryBtn">Salvar avaliação</button></form>}<div className="reviewListV2">{businessReviews.length?businessReviews.map(r=>{const a=reviewAuthors[r.author_id]||{};return <div className="reviewV2" key={r.id}><Avatar profile={a} size="xs"/><div><b>{a.full_name||a.username||'Usuário'}</b><span className="reviewStars">{ratingStars(r.rating)}</span><p>{r.comment||'Sem comentário.'}</p><small>{fmtDate(r.created_at)}</small></div></div>}):<p className="muted">Ainda não há avaliações.</p>}</div></div>
          {mine&&<form className="panel stackForm" onSubmit={updateBusiness}><h2>⚙️ Editar estabelecimento</h2><label>Nome<input value={businessEdit?.name||''} onChange={e=>setBusinessEdit(v=>({...v,name:e.target.value}))}/></label><label>@Usuário<input value={businessEdit?.username||''} onChange={e=>setBusinessEdit(v=>({...v,username:e.target.value}))}/></label><label>Categoria<select value={businessEdit?.category||''} onChange={e=>setBusinessEdit(v=>({...v,category:e.target.value}))}><option value="">Selecione</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select></label><label>Descrição<textarea value={businessEdit?.description||''} onChange={e=>setBusinessEdit(v=>({...v,description:e.target.value}))}/></label><label>Endereço<input value={businessEdit?.address||''} onChange={e=>setBusinessEdit(v=>({...v,address:e.target.value}))}/></label><label>WhatsApp<input value={businessEdit?.whatsapp||''} onChange={e=>setBusinessEdit(v=>({...v,whatsapp:e.target.value}))}/></label><label>Site<input value={businessEdit?.website||''} onChange={e=>setBusinessEdit(v=>({...v,website:e.target.value}))}/></label><label>Logo<input type="file" accept="image/*" onChange={e=>uploadBusinessImage('logo_url',e.target.files?.[0])}/></label><label>Capa<input type="file" accept="image/*" onChange={e=>uploadBusinessImage('cover_url',e.target.files?.[0])}/></label><button className="primaryBtn" disabled={busy}>Salvar estabelecimento</button></form>}
        </div></div>
      </>
    }
    return <><PageHeader title="Estabelecimentos" subtitle={`Descubra negócios, serviços e destaques de ${cityLabel}.`}/><div className="businessIntroV2"><div><span className="eyebrow">COMÉRCIO LOCAL • CIDARANK</span><h2>Descubra o que sua cidade tem de melhor</h2><p>Avaliações reais, seguidores e novidades dos estabelecimentos locais.</p></div><div className="businessIntroStats"><b>{businesses.length}</b><span>estabelecimentos em {profile.city}</span></div></div><div className="businessGrid businessGridV2">{businesses.map(b=><button className="businessCard businessCardV2" key={b.id} onClick={()=>loadBusiness(b.id)}><div className="businessCover" style={b.cover_url?{backgroundImage:`url(${b.cover_url})`}:{}}><span className="businessCardBadge">{b.category||'Estabelecimento local'}</span></div><div className="businessCardBody"><Avatar profile={{avatar_url:b.logo_url}} name={b.name} size="lg"/><div className="grow"><h3>{b.name}</h3><p>📍 {b.city} - {b.state}</p><div className="businessCardRating"><strong>★ {Number(b.rating_average||0).toFixed(1)}</strong><small>{b.reviews_count||0} avaliações</small></div></div></div></button>)}</div>{!businesses.length&&<Empty title="Ainda não há estabelecimentos cadastrados.">Cadastre o primeiro estabelecimento da cidade.</Empty>}
      <form className="panel stackForm businessRegisterV2" onSubmit={saveBusiness}><div className="panelTitle"><div><span className="eyebrow">PARA COMERCIANTES</span><h2>🏪 Cadastre seu estabelecimento</h2><p>O estabelecimento fica vinculado à sua conta e à sua cidade.</p></div></div><div className="formGrid"><label>Nome<input required value={businessForm.name} onChange={e=>setBusinessForm({...businessForm,name:e.target.value})}/></label><label>@Usuário<input required value={businessForm.username} onChange={e=>setBusinessForm({...businessForm,username:e.target.value})}/></label><label>Categoria<select required value={businessForm.category} onChange={e=>setBusinessForm({...businessForm,category:e.target.value})}><option value="">Selecione</option>{CATEGORIES.map(c=><option key={c}>{c}</option>)}</select></label><label>Endereço<input value={businessForm.address} onChange={e=>setBusinessForm({...businessForm,address:e.target.value})}/></label><label>WhatsApp<input value={businessForm.whatsapp} onChange={e=>setBusinessForm({...businessForm,whatsapp:e.target.value})}/></label><label>Site<input value={businessForm.website} onChange={e=>setBusinessForm({...businessForm,website:e.target.value})}/></label></div><label>Descrição<textarea value={businessForm.description} onChange={e=>setBusinessForm({...businessForm,description:e.target.value})}/></label><button className="primaryBtn" disabled={busy}>Cadastrar estabelecimento</button></form>
    </>
  }

  function DirectoryScreen({kind='people'}){
    const isPeople=kind==='people';
    const q=directorySearch.trim().toLowerCase();
    const peopleList=(people||[]).filter(p=>p.id!==session.user.id&&!isInstitutionalOwner(p)).filter(p=>!q||`${p.full_name||''} ${p.username||''} ${p.city||''} ${p.state||''} ${p.activity||''}`.toLowerCase().includes(q));
    const businessList=(businesses||[]).filter(b=>!q||`${b.name||''} ${b.username||''} ${b.category||''} ${b.city||''} ${b.state||''}`.toLowerCase().includes(q));
    return <>
      <PageHeader title={isPeople?'Buscar Pessoas':'Buscar Estabelecimentos'} subtitle={isPeople?`Encontre pessoas de ${cityLabel} e acesse seus perfis.`:`Encontre estabelecimentos de ${cityLabel}.`}/>
      <div className="panel directoryPanel">
        <div className="directorySearchBox">🔎<input autoFocus value={directorySearch} onChange={e=>setDirectorySearch(e.target.value)} placeholder={isPeople?'Pesquisar por nome, @usuário ou atividade...':'Pesquisar por nome, categoria ou @usuário...'}/>{directorySearch&&<button type="button" onClick={()=>setDirectorySearch('')}>×</button>}</div>
        <div className="directoryMeta"><b>{isPeople?peopleList.length:businessList.length}</b><span>{isPeople?'pessoa(s) encontrada(s)':'estabelecimento(s) encontrado(s)'}</span></div>
      </div>
      {isPeople?<div className="directoryGrid">{peopleList.map(p=><button className="directoryCard" key={p.id} onClick={()=>openProfile(p)}><Avatar profile={p} size="lg"/><div className="grow"><b>{p.full_name||p.username}</b><small>@{p.username}</small><span>📍 {p.city||'Cidade'} - {p.state||''}</span>{p.activity&&<em>{p.activity}</em>}</div><strong>Ver perfil →</strong></button>)}{!peopleList.length&&<Empty title="Nenhuma pessoa encontrada.">Tente outro nome ou usuário.</Empty>}</div>
      :<div className="directoryGrid">{businessList.map(b=><button className="directoryCard businessDirectoryCard" key={b.id} onClick={()=>loadBusiness(b.id)}><Avatar profile={{avatar_url:b.logo_url}} name={b.name} size="lg"/><div className="grow"><b>{b.name}</b><small>@{b.username||'estabelecimento'} • {b.category||'Estabelecimento'}</small><span>📍 {b.city||'Cidade'} - {b.state||''}</span><em>★ {Number(b.rating_average||0).toFixed(1)} • {b.reviews_count||0} avaliações</em></div><strong>Ver estabelecimento →</strong></button>)}{!businessList.length&&<Empty title="Nenhum estabelecimento encontrado.">Tente outro nome ou categoria.</Empty>}</div>}
    </>
  }

  function NotificationDropdown(){
    if(!notificationMenuOpen)return null;
    ...
    const preview=n=>n?.metadata?.preview||n?.metadata?.content||n?.metadata?.text||'';
    return <div className="topNotificationDropdown">
      <div className="topNotificationHead"><div><b>🔔 Notificações</b><small>{unreadPersonalNotificationCount} não lida(s)</small></div>{unreadPersonalNotificationCount>0&&<button onClick={markAllNotificationsRead}>Marcar todas como lidas</button>}</div>
      <div className="topNotificationList">{personalNotifications.slice(0,25).map(n=>{const actor=notificationActors[n.actor_id]||{};const detail=preview(n);return <button key={n.id} className={`topNotificationRow ${n.read_at?'':'unread'}`} onClick={()=>{setNotificationMenuOpen(false);markNotificationRead(n)}}><Avatar profile={actor} size="xs"/><span><b>{actor.full_name||actor.username||'Alguém'} {label[n.type]||'interagiu com você'}</b>{detail&&<em>“{detail}”</em>}<small>{fmtDate(n.created_at)}</small></span>{!n.read_at&&<i className="notificationDot"/>}</button>})}{!personalNotifications.length&&<div className="topNotificationEmpty">Nenhuma notificação ainda.</div>}</div>
    </div>
  }
  ...
  function ProfileScreen(){
    ...
    const trophySlots=[
      {key:'week',icon:'🥇',title:'Medalha Semanal',subtitle:'Campeão da semana',active:profileRankPositions.week===1,cls:'trophyWeek'},
      {key:'month',icon:'🏆',title:'Troféu Mensal',subtitle:'Popular do mês',active:profileRankPositions.month===1,cls:'trophyMonth'},
      {key:'year',icon:'👑',title:'Troféu Anual',subtitle:'Popular do ano',active:profileRankPositions.year===1,cls:'trophyYear'},
      {key:'state',icon:'🗺️',title:'Troféu Estadual',subtitle:'EM BREVE',active:false,cls:'trophyState'},
      {key:'brazil',icon:'🇧🇷',title:'Troféu Brasil',subtitle:'EM BREVE',active:false,cls:'trophyBrazil'}
    ];
    const rankCards=[
      {key:'popular',icon:'👑',label:'Popular da Cidade',pos:profileRankPositions.popular},
      {key:'week',icon:'🗳️',label:'Semanal',pos:profileRankPositions.week},
      {key:'month',icon:'🏆',label:'Mensal',pos:profileRankPositions.month},
      {key:'year',icon:'♛',label:'Anual',pos:profileRankPositions.year}
    ];
    ...
    return <>
      ...
      <div className={`profileStats ${ownerProfile?'ownerProfileStats':''}`}><button onClick={()=>openFollowList(p,'followers')}><b>{p?.followers_count||0}</b><span>Seguidores</span></button><button onClick={()=>openFollowList(p,'following')}><b>{p?.following_count||0}</b><span>Seguindo</span></button>{!ownerProfile&&<div><b>{p?.city||'-'}</b><span>Cidade</span></div>}</div>
      {!ownerProfile&&<div className="panel trophyShowcase featuredAchievements"><div className="panelTitle"><div><span className="eyebrow">VITRINE DO PERFIL</span><h2>🏆 Medalhas e Troféus</h2><p>As conquistas ficam em destaque automaticamente quando forem ganhas.</p></div></div><div className="trophyShelf">{trophySlots.map(t=><div key={t.key} className={`trophySlot ${t.cls} ${t.active?'won floatingTrophy':'locked'}`}><span className="trophyBigIcon">{t.icon}</span><b>{t.title}</b><small>{t.active?'CONQUISTADO':t.subtitle}</small></div>)}</div></div>}
      {!ownerProfile&&<div className="panel profileRankingPanel profileRankSpotlight"><div className="profileRankSpotlightHead"><div><span className="eyebrow">AO VIVO • {p?.city||'SUA CIDADE'}</span><h2>📍 Minha posição na cidade</h2><p>Veja rapidamente onde este perfil está em cada ranking.</p></div><div className={`bestRankBadge ${bestRank?.pos<=3?'podium':''}`}><small>MELHOR POSIÇÃO</small><b>{bestRank?`#${bestRank.pos}`:'—'}</b><span>{bestRank?.label||'Ainda não classificado'}</span></div></div><div className="profileRankGrid modernProfileRanks">{rankCards.map(r=><div key={r.key} className={r.pos&&r.pos<=3?'topRank':''}><span>{r.icon} {r.label}</span><b>{r.pos?`#${r.pos}`:'—'}</b><small>{r.pos===1?'🥇 1º LUGAR':r.pos===2?'🥈 2º LUGAR':r.pos===3?'🥉 3º LUGAR':r.pos?'Classificado':'Ainda não classificado'}</small></div>)}</div><div className="profileFutureRanks"><span>🗺️ Estado <b>EM BREVE</b></span><span>🇧🇷 Brasil <b>EM BREVE</b></span></div></div>}
      <div className="panel"><h2>Sobre</h2><p>{p?.bio||'Este perfil ainda não escreveu uma bio.'}</p>{p?.public_phone&&<p>📱 {p.public_phone}</p>}{p?.website&&<p>🌐 {p.website}</p>}</div>
      {ownerProfile&&<div className="panel profileFeedPanel"><div className="panelTitle"><div><h2>📣 Publicações CIDARANK</h2><p>Anúncios, novidades e atualizações oficiais.</p></div>{own&&visiblePosts.length>0&&<button className="dangerBtn" onClick={deleteAllOwnerPosts}>🗑 Limpar tudo</button>}</div><div className="feed">{visiblePosts.length?visiblePosts.map(post=>PostCard({post,authors:profilePostAuthors,interactive:own,officialFeed:true})):<p className="muted">Ainda não há publicações neste perfil.</p>}</div></div>}
      {!canFollow&&!own&&!ownerProfile&&<div className="infoBanner">Você está visitando um perfil de outra cidade. O ranking por seguidores considera conexões locais.</div>}
      {followModal&&<div className="followModalOverlay" onClick={()=>setFollowModal(null)}><div className="followModal" onClick={e=>e.stopPropagation()}><div className="followModalHead"><h3>{followModal==='followers'?'Seguidores':'Seguindo'}</h3><button onClick={()=>setFollowModal(null)}>×</button></div><div className="directorySearchBox">🔎<input value={followSearch} onChange={e=>setFollowSearch(e.target.value)} placeholder="Pesquisar pessoa..."/></div><div className="followList">{followList.filter(x=>`${x.full_name||''} ${x.username||''}`.toLowerCase().includes(followSearch.toLowerCase())).map(x=><button key={x.id} onClick={()=>{setFollowModal(null);openProfile(x)}}><Avatar profile={x} size="sm"/><div><b>{x.full_name||x.username}</b><small>@{x.username}</small></div></button>)}</div></div></div>}
    </>
  }
  ...
  function MessagesScreen(){
    ...
    const chatMessages=selectedChat?daquitop_messages.filter(m=>(m.sender_id===session.user.id&&m.recipient_id===selectedChat.id)||(m.sender_id===selectedChat.id&&m.recipient_id===session.user.id)).sort((a,b)=>new Date(a.created_at)-new Date(b.created_at)):[];
    const businessForPerson=p=>(businesses||[]).find(b=>b.owner_id===p?.id);
    const kindFor=p=>businessForPerson(p)?'business':(cityKey(p?.city)===cityKey(profile?.city)&&stateKey(p?.state)===stateKey(profile?.state)?'local':'visitor');
    const tabOk=p=>messageTab==='all'||kindFor(p)===messageTab;
    const searchOk=p=>!messageSearch.trim()||`${p?.full_name||''} ${p?.username||''} ${p?.city||''} ${p?.state||''} ${businessForPerson(p)?.name||''}`.toLowerCase().includes(messageSearch.trim().toLowerCase());
    const visibleIds=convoIds.filter(id=>tabOk(messagePeople[id]||{})&&searchOk(messagePeople[id]||{}));
    const starters=people.filter(p=>p.id!==session.user.id&&tabOk(p)&&searchOk(p)).slice(0,30);
    const identity=p=>{const biz=businessForPerson(p);if(biz)return {name:biz.name,sub:`🏪 Estabelecimento • ${biz.city||p.city||''} - ${biz.state||p.state||''}`,avatar:{avatar_url:biz.logo_url}};const visitor=kindFor(p)==='visitor';return {name:p.full_name||p.username||'Usuário',sub:`${visitor?'✈ Visitante':'📍 Da sua cidade'} • ${p.city||'Cidade'} - ${p.state||'UF'}`,avatar:p}};
    const lastPreview=m=>m?.content?.trim()?m.content:(m?.media_type==='video'?'🎥 Vídeo':m?.media_url?'📷 Foto':'Mensagem');
    return <>{exploreCity&&<div className="visitChatReturn"><button onClick={()=>setActive('Explorar Cidade')}>← Voltar para {exploreCity}</button><span>Você está visitando {exploreCity} - {exploreState}</span></div>}<div className="messageBackRow"><button className="backBtn" onClick={()=>setActive(exploreCity?'Explorar Cidade':'Início')}>← Voltar</button></div><PageHeader title="Direct CIDARANK" subtitle="Converse com pessoas e estabelecimentos de qualquer cidade do Brasil."/><div className="directShell"><aside className="directSidebar"><div className="directTitle"><div><b>Mensagens</b><small>{convoIds.length} conversa(s)</small></div><span>💬</span></div><div className="directSearch">🔎 <input value={messageSearch} onChange={e=>setMessageSearch(e.target.value)} placeholder="Pesquisar conversa..."/></div><div className="directTabs"><button className={messageTab==='all'?'active':''} onClick={()=>setMessageTab('all')}>Todas</button><button className={messageTab==='local'?'active':''} onClick={()=>setMessageTab('local')}>Da cidade</button><button className={messageTab==='business'?'active':''} onClick={()=>setMessageTab('business')}>Estabelecimentos</button><button className={messageTab==='visitor'?'active':''} onClick={()=>setMessageTab('visitor')}>Visitantes</button></div><div className="directList">{visibleIds.map(id=>{const p=messagePeople[id]||{};const last=conversations[id];const unread=last.recipient_id===session.user.id&&!last.read_at;const meta=identity(p);return <button className={`directConversation ${selectedChat?.id===id?'active':''}`} key={id} onClick={()=>openChat(p)}><Avatar profile={meta.avatar}/><div className="grow"><div className="directName"><b>{meta.name}</b><time>{new Date(last.created_at).toLocaleTimeString('pt-BR',{hour:'2-digit',minute:'2-digit'})}</time></div><small>{meta.sub}</small><p>{lastPreview(last).slice(0,55)}</p></div>{unread&&<i/>}</button>})}{!visibleIds.length&&<div className="directEmpty">Nenhuma conversa neste filtro.</div>}<div className="directStartTitle">NOVA CONVERSA</div>{starters.map(p=>{const meta=identity(p);return <button className="directConversation compact" key={p.id} onClick={()=>openChat(p)}><Avatar profile={meta.avatar}/><div className="grow"><b>{meta.name}</b><small>{meta.sub}</small></div></button>})}</div></aside><main className={`directChat ${selectedChat?'open':''}`}>{selectedChat?(()=>{const meta=identity(selectedChat);const biz=businessForPerson(selectedChat);return <><header className="directChatHead"><button className="directMobileBack" onClick={()=>setSelectedChat(null)}>←</button><Avatar profile={meta.avatar}/><div className="grow"><b>{meta.name}</b><small>{meta.sub}</small></div>{biz?<button className="directProfileAction" onClick={()=>loadBusiness(biz.id)}>Ver estabelecimento</button>:<button className="directProfileAction" onClick={()=>openProfile(selectedChat)}>Ver perfil</button>}</header><div className="directMessages">{chatMessages.map(m=><div className={`directMessage ${m.sender_id===session.user.id?'mine':''}`} key={m.id}>{m.sender_id!==session.user.id&&<Avatar profile={meta.avatar} size="xs"/>}<div className="directBubble">{m.media_url&&<div className="directMedia">{m.media_type==='video'?<video src={m.media_url} controls playsInline preload="metadata"/>:<img src={m.media_url} alt="Foto enviada na conversa"/>}</div>}{m.content&&<p>{m.content}</p>}<small>{fmtDate(m.created_at)} {m.sender_id===session.user.id&&(m.read_at?' • Visto':' • Enviado')}</small></div></div>)}</div><form className="directComposer modernDirectComposer" onSubmit={sendMessage}>{messageMediaPreview&&<div className="directMediaPreview">{messageMediaType==='video'?<video src={messageMediaPreview} controls muted/>:<img src={messageMediaPreview} alt="Prévia"/>}<button type="button" onClick={clearMessageMedia}>×</button></div>}<div className="directComposerRow"><input ref={messageFileRef} type="file" accept="image/*,video/*" hidden onChange={e=>chooseMessageMedia(e.target.files?.[0])}/><button type="button" className="directToolBtn" title="Enviar foto ou vídeo" onClick={()=>messageFileRef.current?.click()}>＋</button><div className="directEmojiWrap"><button type="button" className="directToolBtn" title="Emojis" onClick={()=>setMessageEmojiOpen(v=>!v)}>☺</button>{messageEmojiOpen&&<div className="directEmojiPicker">{DIRECT_EMOJIS.map(e=><button type="button" key={e} onClick={()=>setMessageText(v=>`${v}${e}`)}>{e}</button>)}</div>}</div><input value={messageText} onChange={e=>setMessageText(e.target.value)} placeholder={`Mensagem para ${meta.name}...`}/><button className="directSend" disabled={messageMediaUploading||(!messageText.trim()&&!messageMediaFile)}>{messageMediaUploading?'Enviando...':'Enviar'}</button></div></form></>})():<div className="directWelcome"><span>💬</span><h2>Seu Direct CIDARANK</h2><p>Escolha uma conversa ou procure uma pessoa ou estabelecimento.</p><small>Envie texto, emojis, fotos e vídeos.</small></div>}</main></div></>
  }
  ...
  async function ownerPrepareLaunch(){
    if(!isOwner)return;
    if(launchResetText!=='LANÇAR CIDARANK'){alert('Digite exatamente LANÇAR CIDARANK para confirmar.');return;}
    if(!confirm('ATENÇÃO: esta ação remove contas e dados de TESTE e mantém a conta do Dono e a estrutura do CIDARANK. Deseja continuar?'))return;
    setLaunchResetBusy(true);
    const {data,error}=await supabase.rpc('cidarank_prepare_launch',{p_confirmation:launchResetText});
    setLaunchResetBusy(false);
    if(error){alert(`Não foi possível preparar o lançamento: ${error.message}`);return;}
    setLaunchResetText('');
    await logAdminAction('launch_cleanup_completed','system',null,data||{});
    await loadOwnerData();
    setNotice('🚀 CIDARANK preparado para lançamento. Dados de teste zerados e conta do Dono preservada.');
  }
  ...
  function OwnerPanelScreen(){
    ...
    return <>
      ...
      <div className="ownerTabs">{(isOwner?[
        ['dashboard','📊 Visão geral'],['tempo','⚡ Tempo real'],['usuarios','👥 Usuários'],['moderacao','🛡️ Moderação'],['comercios','🏪 Estabelecimentos'],['rankings','🏆 Rankings'],['suporte','🎫 Suporte'],['equipe','👑 Equipe'],['expansao','🌎 Expansão'],['comunicados','📢 Comunicados'],['banner','🖼️ Banner'],['auditoria','📜 Auditoria'],['lancamento','🧹 Preparar lançamento'],['sistema','⚙️ Sistema']
      ...
      {ownerTab==='expansao'&&isOwner&&<div className="panel expansionPanel">
        <div className="ownerTicketHead"><div><span className="eyebrow">CONTROLE EXCLUSIVO DO DONO</span><h2>🌎 Expansão do CIDARANK</h2><p className="muted">Abra cidades aos poucos. O cadastro só mostra estados e cidades ATIVOS. ADMs não podem alterar esta área.</p></div><div className="expansionStats"><b>{expansionRows.filter(x=>x.is_open).length}</b><small>cidades ativas</small><b>{waitlistCount}</b><small>aguardando liberação</small></div></div>
        <div className="expansionControls"><select value={expansionStateId} onChange={handleExpansionStateChange}><option value="">Escolha um estado</option>{states.map(st=><option key={st.id} value={st.id}>{st.nome} ({st.sigla})</option>)}</select><div className="expansionSearch">🔎<input value={expansionCitySearch} onChange={e=>setExpansionCitySearch(e.target.value)} placeholder="Pesquisar cidade..." disabled={!expansionStateId}/></div><button className="safeBtn" disabled={!expansionStateId||expansionBusy} onClick={()=>ownerSetStateOpen(true)}>🟢 Liberar estado inteiro</button><button className="dangerBtn" disabled={!expansionStateId||expansionBusy} onClick={()=>ownerSetStateOpen(false)}>🔒 Pausar estado inteiro</button></div>
        {expansionStateId&&<><div className="expansionSyncInfo">🔄 Sincronizado com o cadastro: somente cidades marcadas como <b>ATIVA</b> aparecem para novos usuários.</div><div className="expansionCityGrid">{filteredExpansionCities.map(c=>{const row=expansionRows.find(x=>stateKey(x.state)===stateKey(expansionState)&&cityKey(x.city)===cityKey(c.nome));const open=Boolean(row?.is_open);return <div className={`expansionCity ${open?'open':'closed'}`} key={c.id}><div><b>{c.nome}</b><small>{expansionState} • {open?'ATIVA':'EM BREVE'}</small></div><button className={open?'dangerBtn':'safeBtn'} onClick={()=>ownerSetCityOpen(c.nome,!open)}>{open?'Pausar':'Ativar'}</button></div>})}{!filteredExpansionCities.length&&<div className="expansionNoResults">Nenhuma cidade encontrada.</div>}</div></>}
        <section className="scopeCompetitionAdmin"><div className="scopeCompetitionIntro"><span className="eyebrow">CIDADE → ESTADO → BRASIL</span><h3>🏁 Chaves das competições superiores</h3><p>Ao liberar, Ranking e Tabela de Classificação são ativados juntos. O Estadual puxa automaticamente os TOP 10 anuais de cada cidade do estado. O Brasil puxa os TOP 10 de cada Estadual.</p></div><div className="scopeCompetitionGrid"><div className={`scopeControlCard ${scopeCompetitionEnabled('state',expansionState)?'live':''}`}><div><b>🗺️ Estadual {expansionState||'—'}</b><small>{expansionState?(scopeCompetitionEnabled('state',expansionState)?`${scopeCompetitionOpen('state',expansionState)?'AO VIVO':'ENCERRADO'} • encerra ${scopeEndLabel('state',expansionState)}`:'EM BREVE • pronto para liberar'):'Escolha um estado acima'}</small></div><label>Encerramento<input type="datetime-local" value={ownerStateCompetitionEnd} onChange={e=>setOwnerStateCompetitionEnd(e.target.value)} disabled={!expansionState}/></label><button className={scopeCompetitionEnabled('state',expansionState)?'dangerBtn':'safeBtn'} disabled={!expansionState||expansionBusy} onClick={()=>ownerSetScopeCompetition('state',!scopeCompetitionEnabled('state',expansionState))}>{scopeCompetitionEnabled('state',expansionState)?'⏸ Pausar Estadual':'🟢 Liberar Estadual'}</button><p>Classificados: <b>10 melhores do Anual de cada cidade</b>. Podem votar todos os usuários do mesmo estado.</p></div><div className={`scopeControlCard ${scopeCompetitionEnabled('brazil')?'live':''}`}><div><b>🇧🇷 Brasil</b><small>{scopeCompetitionEnabled('brazil')?`${scopeCompetitionOpen('brazil')?'AO VIVO':'ENCERRADO'} • encerra ${scopeEndLabel('brazil')}`:'EM BREVE • pronto para liberar'}</small></div><label>Encerramento<input type="datetime-local" value={ownerBrazilCompetitionEnd} onChange={e=>setOwnerBrazilCompetitionEnd(e.target.value)}/></label><button className={scopeCompetitionEnabled('brazil')?'dangerBtn':'safeBtn'} disabled={expansionBusy} onClick={()=>ownerSetScopeCompetition('brazil',!scopeCompetitionEnabled('brazil'))}>{scopeCompetitionEnabled('brazil')?'⏸ Pausar Brasil':'🟢 Liberar Brasil'}</button><p>Classificados: <b>10 melhores de cada Estado</b>. Podem votar usuários de qualquer estado do Brasil.</p></div></div><div className="scopeSyncLegend"><span>✅ Ranking sincronizado</span><span>✅ Tabela sincronizada</span><span>✅ Classificados congelados ao liberar</span><span>🔒 Estadual/Brasil: 1 voto definitivo por conta e edição</span></div></section>
        <div className="emergencyBox"><div><b>🛡️ Chave de emergência das publicações</b><small>Use somente em emergência. O conteúdo existente continua visível.</small></div><button className={postingPaused?'safeBtn':'dangerBtn'} onClick={ownerTogglePostingPause}>{postingPaused?'▶ Reativar novas publicações':'⏸ Pausar novas publicações'}</button></div>
      </div>}
      ...
      {ownerTab==='lancamento'&&isOwner&&<div className="panel launchResetPanel"><span className="eyebrow">SOMENTE O DONO</span><h2>🧹 Preparar CIDARANK para Lançamento</h2><p className="muted">Use apenas quando os testes terminarem. A limpeza remove contas comuns de teste e seus dados vinculados, zera votos/rankings de teste e preserva a conta do Dono, configurações, cidades liberadas, banners e estrutura do sistema.</p><div className="launchKeepGrid"><span>✅ Preserva Dono</span><span>✅ Preserva configurações</span><span>✅ Preserva cidades/estados</span><span>✅ Preserva banners e regras</span><span>🧹 Remove contas de teste</span><span>🧹 Zera dados vinculados aos testes</span></div><div className="dangerLaunchBox"><b>⚠️ AÇÃO IRREVERSÍVEL</b><p>Digite <strong>LANÇAR CIDARANK</strong> para liberar o botão. Faça backup do Supabase antes da limpeza final.</p><input value={launchResetText} onChange={e=>setLaunchResetText(e.target.value)} placeholder="Digite LANÇAR CIDARANK"/><button className="dangerBtn" disabled={launchResetText!=='LANÇAR CIDARANK'||launchResetBusy} onClick={ownerPrepareLaunch}>{launchResetBusy?'Limpando...':'🧹 LIMPAR TESTES E PREPARAR LANÇAMENTO'}</button></div></div>}
  ...
  function RulesScreen(){
    return <><PageHeader title="Regras do CIDARANK" subtitle="Entenda de forma simples como funcionam rankings, conquistas, estabelecimentos e a comunidade."/>
      <div className="rulesHero panel"><span className="eyebrow">COMO FUNCIONA</span><h2>📜 Regras oficiais do CIDARANK</h2><p>As mesmas regras valem para todas as cidades liberadas no Brasil. A plataforma usa ciclos automáticos e mantém cada cidade vinculada à sua própria comunidade.</p></div>
      <div className="rulesGrid">
        <section className="panel ruleCard"><h3>🗳️ Ranking Semanal</h3><p>É o ranking de votação direta. Cada ciclo usa uma chave semanal própria, por isso a nova semana começa automaticamente sem misturar os votos da anterior.</p><b>Encerra: {periodEndLabel('week')}</b><small>Tempo restante: {periodCountdown('week')}</small></section>
        <section className="panel ruleCard"><h3>🏆 Ranking Mensal</h3><p>É calculado automaticamente pelos resultados das semanas do mês. As melhores colocações semanais geram pontos para o mês.</p><b>Encerra: {periodEndLabel('month')}</b><small>Tempo restante: {periodCountdown('month')}</small></section>
        <section className="panel ruleCard"><h3>👑 Ranking Anual</h3><p>É formado automaticamente pelo desempenho acumulado dos meses do ano. Ao iniciar um novo ano, começa um novo ciclo anual.</p><b>Encerra: {periodEndLabel('year')}</b><small>Tempo restante: {periodCountdown('year')}</small></section>
        <section className="panel ruleCard"><h3>⭐ Popular da Cidade</h3><p>É permanente e baseado em seguidores válidos. Não zera semanalmente, mensalmente ou anualmente. Também aparece na <b>Tabela de Classificação</b> e fica sincronizado com o Ranking da Cidade.</p><b>Atualização contínua</b><small>Exclusivo da cidade de cadastro.</small></section>
        <section className="panel ruleCard"><h3>🗺️ Etapa Estadual</h3><p>Quando o Dono liberar a etapa, entram automaticamente os <b>10 melhores do Ranking Anual de cada cidade</b> daquele estado. Usuários de qualquer cidade do mesmo estado podem votar em qualquer classificado estadual. Cada conta tem 1 voto definitivo por edição estadual. Antes de confirmar, confira sua escolha: depois do registro o voto não pode ser trocado.</p><b>{scopeCompetitionEnabled('state',profile?.state)?(scopeCompetitionOpen('state',profile?.state)?'AO VIVO':'ENCERRADO'):'EM BREVE'}</b><small>{scopeCompetitionEnabled('state',profile?.state)?`Encerra: ${scopeEndLabel('state')}`:'A liberação é feita pelo Painel do Dono.'}</small></section>
        <section className="panel ruleCard"><h3>🇧🇷 Etapa Brasil</h3><p>Quando liberada, entram automaticamente os <b>10 melhores classificados de cada Estado</b>. Na fase Brasil, usuários de qualquer estado podem votar em qualquer participante nacional classificado. Cada conta tem 1 voto definitivo por edição nacional e não pode trocar depois de confirmar.</p><b>{scopeCompetitionEnabled('brazil')?(scopeCompetitionOpen('brazil')?'AO VIVO':'ENCERRADO'):'EM BREVE'}</b><small>{scopeCompetitionEnabled('brazil')?`Encerra: ${scopeEndLabel('brazil')}`:'A liberação é feita pelo Painel do Dono.'}</small></section>
        <section className="panel ruleCard"><h3>🏁 Caminho até o Brasil</h3><p><b>Cidade:</b> Semanal → Mensal → Anual. <b>Estado:</b> TOP 10 anual de cada cidade. <b>Brasil:</b> TOP 10 final de cada Estado. Ranking e Tabela de Classificação usam os mesmos dados e devem mostrar a mesma ordem.</p><b>Progressão automática quando cada fase é liberada</b></section>
        <section className="panel ruleCard"><h3>🏅 Medalhas e troféus</h3><p>Vitória semanal gera medalha. Vitória mensal e anual gera troféu correspondente. As conquistas ficam no perfil e não devem desaparecer quando um novo ciclo começar.</p></section>
        <section className="panel ruleCard"><h3>🏪 Estabelecimentos</h3><p>A nota vem da média das avaliações válidas de 1 a 5 estrelas. Seguidores e curtidas medem popularidade e engajamento, mas não alteram artificialmente a nota de estrelas.</p></section>
        <section className="panel ruleCard"><h3>📣 Feed e marcações</h3><p>O Feed CIDARANK oficial tem alcance nacional. Marcações entre usuários são locais: a pessoa só pode marcar perfis da mesma cidade de cadastro.</p></section>
        <section className="panel ruleCard"><h3>💬 Mensagens e notificações</h3><p>Mensagens, curtidas, comentários e marcações podem gerar notificações. Ao tocar em uma notificação, o destino deve levar ao conteúdo ou conversa correspondente.</p></section>
        <section className="panel ruleCard"><h3>🛡️ Suporte e conduta</h3><p>Denúncias e chamados podem ser analisados pela Administração. Conteúdo impróprio pode ser ocultado e contas podem ser suspensas conforme a gravidade e as regras da comunidade.</p></section>
        <section className="panel ruleCard"><h3>🌎 Expansão</h3><p>Cadastro e Explorar Cidade mostram somente estados e cidades liberados pelo Dono. Ao ativar ou pausar uma cidade, a disponibilidade pública é sincronizada.</p></section>
      </div>
    </>
  }
  ...
  function renderScreen(){
    switch(active){
      ...
      case 'Regras do CIDARANK': return RulesScreen();
