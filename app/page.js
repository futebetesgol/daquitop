...
const MENU = [
  ['⌂','Início'],['🏆','Ranking da Cidade'],['▰','Estabelecimentos'],['⌖','Explorar Cidade'],['🔎','Buscar Pessoas'],['🏪','Buscar Estabelecimentos'],['🔔','Notificações'],['📣','Feed do Dono'],['✈','Mensagens'],['●','Perfil'],['⚙','Configurações'],['?','Suporte']
];
...
export default function Home(){
  ...
  async function loadOwnerPublicFeed(){
    ...
    const {data:raw}=await supabase.from('posts').select('*').eq('author_id',op.id).order('created_at',{ascending:false}).limit(50);
  ...
  async function loadSiteSettings(){
    try{
      const visualKeys=['national_banner','national_podium_banner','podium_banner_city','podium_banner_week','podium_banner_month','podium_banner_year','podium_banner_state','podium_banner_brazil'];
  ...
  async function loadProfilePosts(person){
    ...
    const {data}=await supabase.from('posts').select('*').eq('author_id',person.id).order('created_at',{ascending:false}).limit(30);
  ...
  async function openOwnProfile(){setSelectedProfile(profile);await Promise.all([loadProfileAwards(profile),loadProfilePosts(profile)]);setActive('Perfil')}
  ...
  const notificationCount=useMemo(()=>{
    const unreadMessages=daquitop_messages.filter(m=>m.recipient_id===session?.user?.id&&!m.read_at).length;
    const seen=lastSeenCityFeed?new Date(lastSeenCityFeed).getTime():0;
    const newPosts=posts.filter(p=>new Date(p.created_at).getTime()>seen&&p.author_id!==session?.user?.id).length;
    return unreadMessages+newPosts;
  },[daquitop_messages,posts,lastSeenCityFeed,session?.user?.id]);
  ...
  function OwnerFeedScreen(){
    return <><PageHeader title="Feed do Dono" subtitle="Anúncios, atualizações e publicações oficiais do CIDARANK." actions={ownerPublicProfile&&<button className="secondaryBtn" onClick={()=>openProfile(ownerPublicProfile)}>Ver perfil do Dono</button>}/>{isOwner&&Composer({text:postText,setText:setPostText,onPublish:async()=>{await publishPost();await loadOwnerPublicFeed()}})}<div className="feed">{ownerPublicPosts.length?ownerPublicPosts.map(p=>PostCard({post:p,authors:ownerPublicAuthors})):<Empty title="Ainda não há publicações oficiais.">O Dono pode publicar novidades aqui.</Empty>}</div></>
  }
  ...
  function renderScreen(){
    switch(active){
      ...
      case 'Notificações': return NotificationsScreen();
      ...
      case 'Feed do Dono': return OwnerFeedScreen();
