...
export default function Home(){
  ...
  // Relógio do ranking: mantém data/hora e contagem regressiva atualizadas na tela.
  useEffect(()=>{
    const timer=setInterval(()=>setClockNow(Date.now()),30000);
    return()=>clearInterval(timer);
  },[]);

  // Ranking local em atualização automática. Recarrega periodicamente e também
  // escuta mudanças de votos da cidade quando o Realtime do Supabase está ativo.
  useEffect(()=>{
    if(!session?.user?.id || !profile?.city || !profile?.state || active!=='Ranking da Cidade') return;
    let stopped=false;
    const refresh=()=>{ if(!stopped) loadCompetitions(profile.city,profile.state); };
    refresh();
    const timer=setInterval(refresh,15000);
    const channel=supabase.channel(`cidarank-ranking-live-${session.user.id}-${cityKey(profile.city)}`)
      .on('postgres_changes',{event:'*',schema:'public',table:'daquitop_competition_votes',filter:`city_key=eq.${cityKey(profile.city)}`},refresh)
      .subscribe();
    return()=>{ stopped=true; clearInterval(timer); supabase.removeChannel(channel); };
  },[session?.user?.id,profile?.city,profile?.state,active]);
  ...
  function RankingCityScreen(){
    const isFutureScope=['state','brazil'].includes(popularView);
    ...
    const rank=isFutureScope?[]:rankType?competitionRanking(rankType):popularView==='city'?popularRank:[];
    ...
    const metricValue=p=>popularView==='city'?Number(p.followers_count||0):rankType==='week'?Number(p.votes||0):Number(p.points||0);
    const metric=p=>popularView==='city'?`${p.followers_count||0} seguidores`:rankType==='week'?`${p.votes||0} voto(s)`:`${p.points||0} pontos`;
    const status=isFutureScope
      ? {label:popularView==='state'?'RANKING ESTADUAL':'RANKING BRASIL',countdown:'EM BREVE',icon:popularView==='state'?'🗺️':'🇧🇷'}
      : rankType?rankStatus(rankType):{label:'POPULARIDADE PERMANENTE',countdown:'Não reinicia',icon:'👑'};
    const title=popularView==='state'?'Ranking Estadual':popularView==='brazil'?'Ranking Brasil':popularView==='city'?'Mais Popular da Cidade':rankType==='week'?'Ranking Semanal':rankType==='month'?'Ranking Mensal':'Ranking Anual';
    const scopeLabel=popularView==='state'?`${profile?.state||'Seu estado'}`:popularView==='brazil'?'Brasil':cityLabel;
    const nextAbove=myPos>0?rank[myPos-1]:null;
    const gapToNext=myPos>0&&nextAbove?Math.max(0,metricValue(nextAbove)-metricValue(rank[myPos])):0;
    const myRankCard=isFutureScope
      ? <div className="myRankSpot myRankSpotFuture"><span>📍 SUA POSIÇÃO</span><b>—</b><small>{popularView==='state'?'Ranking Estadual':'Ranking Brasil'} • EM BREVE</small></div>
      : myPos>=0
        ? <div className="myRankSpot"><span>📍 SUA POSIÇÃO AGORA</span><b>#{myPos+1}</b><small>{metric(rank[myPos])} • {rank.length} classificado(s){myPos===0?' • 🥇 você está liderando':gapToNext>0?` • faltam ${gapToNext} ${popularView==='city'?'seguidor(es)':rankType==='week'?'voto(s)':'ponto(s)'} para alcançar #${myPos}`:''}</small></div>
        : <div className="myRankSpot"><span>📍 SUA POSIÇÃO AGORA</span><b>—</b><small>Ainda não classificado neste ranking.</small></div>;
    ...
    return <><PageHeader title="Ranking" subtitle={`Acompanhe posições locais, estaduais e nacionais do CIDARANK.`}/>
      <div className="panel rankingChooser"><div className="segmented wrap">
        <button className={popularView==='city'?'active':''} onClick={()=>setPopularView('city')}>Popular da cidade</button>
        <button className={popularView==='week'?'active':''} onClick={()=>setPopularView('week')}>Semanal</button>
        <button className={popularView==='month'?'active':''} onClick={()=>setPopularView('month')}>Mensal</button>
        <button className={popularView==='year'?'active':''} onClick={()=>setPopularView('year')}>Anual</button>
        <button className={popularView==='state'?'active futureTab':''} onClick={()=>setPopularView('state')}>Estado • EM BREVE</button>
        <button className={popularView==='brazil'?'active futureTab':''} onClick={()=>setPopularView('brazil')}>Brasil • EM BREVE</button>
        <button className={popularView==='business'?'active':''} onClick={()=>setPopularView('business')}>Estabelecimentos</button>
      </div></div>
      <div className="panel rankHow"><h2>Como funciona?</h2><p><b>Semanal:</b> votação direta e atualização automática. <b>Mensal:</b> nasce dos resultados das semanas do mês. <b>Anual:</b> nasce dos resultados mensais. O fechamento cria um novo ciclo sem apagar o histórico anterior. <b>Estado e Brasil:</b> já aparecem na estrutura, mas ficam como EM BREVE até a liberação oficial.</p><div className="rankFlow"><span>🗳️ SEMANAL</span><b>→</b><span>🏆 MENSAL</span><b>→</b><span>👑 ANUAL</span><b>•</b><span className="futureMini">🗺️ ESTADO EM BREVE</span><b>•</b><span className="futureMini">🇧🇷 BRASIL EM BREVE</span></div></div>
      {popularView!=='business'&&<>
        <section className="panel mainRankStage"><div className="panelTitle"><div><span className="eyebrow">AO VIVO • {scopeLabel}</span><h2>{title}</h2><p>{isFutureScope?'Estrutura preparada para a próxima etapa nacional.':'TOP 3 em destaque e classificação completa logo abaixo.'}</p></div><span className="livePill">● {isFutureScope?'EM BREVE':'ATUALIZAÇÃO AUTOMÁTICA'}</span></div>
          <div className="rankStatusBar"><span>{status.icon}</span><div><b>{status.label}</b><small>{isFutureScope?<strong>EM BREVE</strong>:rankType?<>Encerra em <strong>{periodEndLabel(rankType)}</strong> • faltam <strong>{status.countdown}</strong></>:<>Status: <strong>{status.countdown}</strong></>}</small></div></div>
          <div className="floatingPodium rankPagePodium podiumWithNationalBg" style={{'--podium-bg':`url(${bannerForRank(popularView)})`}}>{isFutureScope?<div className="rankEmptyState podiumEmptyOnBanner"><span>🔒</span><b>{title} — EM BREVE</b><small>A tabela já está preparada e será liberada quando esta etapa do CIDARANK abrir oficialmente.</small></div>:top3.length?order.map(p=>{const pos=rank.findIndex(x=>x.id===p.id)+1;return <button className={`floatingWinner place${pos} animatedRankPerson`} key={p.id} onClick={()=>openProfile(p)}><span className="floatingPlace">#{pos}</span><span className="floatingCrown">{pos===1?'♛':pos===2?'◆':'▲'}</span><span className="rankPhotoSquare"><Avatar profile={p} name={p.full_name} size={pos===1?'lg':'md'}/></span><div className="winnerIdentity"><b>{p.full_name||p.username}</b><Seal type={sealTypeFor(p)} color={p.seal_color}/></div><small>@{p.username}</small><strong>{metric(p)}</strong><span className="podiumBase">{pos}º</span></button>}):<div className="rankEmptyState podiumEmptyOnBanner"><span>🏆</span><b>Ainda não há classificados</b><small>Assim que houver votos, pontos ou seguidores válidos, o TOP 3 aparece automaticamente.</small></div>}</div>
        </section>

        <div className="panel fullRanking"><div className="panelTitle"><div><h2>📊 Tabela de classificação</h2><p>{isFutureScope?'Tabela reservada para a próxima etapa.':'Posições em tempo real do #1 em diante.'}</p></div><b>{isFutureScope?'EM BREVE':`${rank.length} classificados`}</b></div>
          {isFutureScope?<div className="futureRankingTable"><div className="listRow"><b className="rankNum">#</b><div className="grow"><b>{title}</b><small>A classificação será ativada automaticamente quando este nível for liberado.</small></div><strong>🔒 EM BREVE</strong></div></div>:<div className="listTable">{rank.map((p,i)=><div className={`listRow ${p.id===session.user.id?'myRankRow':''}`} key={p.id}><b className="rankNum">#{i+1}</b><button className="plainBtn" onClick={()=>openProfile(p)}><Avatar profile={p}/></button><div className="grow"><div className="nameWithSeal"><button className="linkName" onClick={()=>openProfile(p)}>{p.full_name||p.username}</button><Seal type={sealTypeFor(p)} color={p.seal_color}/>{p.id===session.user.id&&<span className="youPill">VOCÊ</span>}</div><small>@{p.username}</small></div><strong>{metric(p)}</strong></div>)}{!rank.length&&<p className="muted">Ainda não há classificados neste ranking.</p>}</div>}
          {myRankCard}
        </div>
    ...
    if(selectedBusiness){
      ...
      return <><button className="backBtn" onClick={()=>setSelectedBusiness(null)}>← Voltar para estabelecimentos</button>
        ...
        <div className="twoCol businessTwoCol"><div>
          ...
          <div className="panel"><div className="panelTitle"><div><h2>📰 Novidades do estabelecimento</h2><p>Publicações, produtos, serviços e avisos.</p></div></div>{businessPosts.length?<div className="feed">{businessPosts.map(p=>PostCard({post:p,authors:businessPostAuthors,bizMap:{[selectedBusiness.id]:selectedBusiness},interactive:false}))}</div>:<p className="muted">Este estabelecimento ainda não publicou nada.</p>}</div>
        </div><div>
