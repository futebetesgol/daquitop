...
export default function Home(){
  ...
  const [sitePodiumBanner,setSitePodiumBanner]=useState('/cidarank-podium-bg.webp');
  ...
  async function loadSiteSettings(){
    try{
      const {data,error}=await supabase.from('cidarank_site_settings').select('key,value').in('key',['national_banner','national_podium_banner']);
      if(!error){
        const map=Object.fromEntries((data||[]).map(x=>[x.key,x.value]));
        if(map.national_banner)setSiteBanner(map.national_banner);
        if(map.national_podium_banner)setSitePodiumBanner(map.national_podium_banner);
      }
    }catch(e){
  ...
  async function changePodiumBanner(file){
    if(!file || !isOwner)return;
    setOwnerBusy(true);
    try{
      const url=await uploadMedia(file,'podium-banner');
      const {error}=await supabase.from('cidarank_site_settings').upsert({
        key:'national_podium_banner',
        value:url,
        updated_by:session.user.id,
        updated_at:new Date().toISOString()
      },{onConflict:'key'});
      if(error)throw error;
      setSitePodiumBanner(url);
      await logAdminAction('podium_banner_updated','site','national_podium_banner',{url});
      setNotice('Imagem nacional do pódio atualizada para todas as cidades.');
    }catch(e){
      alert(`Não foi possível trocar a imagem do pódio: ${e.message}`);
    }
    setOwnerBusy(false);
  }
  ...
  function CityHighlights(){
    ...
    const views={week:{title:'TOP 3 DA SEMANA',list:weekly,metric:'votes',icon:'🏆'},month:{title:'TOP 3 DO MÊS',list:monthly,metric:'points',icon:'🥇'},year:{title:'TOP 3 DO ANO',list:yearly,metric:'points',icon:'👑'},city:{title:'MAIS POPULARES',list:popular,metric:'followers',icon:'⭐'}};
  ...
  function OwnerPanelScreen(){
    ...
    return <>
      ...
      {ownerTab==='banner'&&<><div className="panel"><h2>Banner nacional do CIDARANK</h2><p className="muted">A imagem é nacional e somente o dono principal pode trocar.</p><div className="ownerBannerPreview"><img src={siteBanner||'/cidarank-national-banner.png'} alt="Banner nacional atual"/></div>{isOwner?<label className="primaryBtn ownerUpload">Trocar imagem nacional<input type="file" hidden accept="image/*" disabled={ownerBusy} onChange={e=>changeNationalBanner(e.target.files?.[0])}/></label>:<div className="infoBanner">Apenas o dono pode trocar o banner.</div>}</div><div className="panel podiumBannerAdmin"><h2>🏆 Fundo nacional do pódio</h2><p className="muted">Este fundo aparece atrás do Top 1, 2 e 3 em todas as cidades. Recomendado: 1600 × 600 px, WebP/JPG otimizado.</p><div className="ownerBannerPreview podiumPreview"><img src={sitePodiumBanner||'/cidarank-podium-bg.webp'} alt="Fundo nacional do pódio"/></div>{isOwner&&<label className="primaryBtn ownerUpload">Trocar fundo do pódio<input type="file" hidden accept="image/*" disabled={ownerBusy} onChange={e=>changePodiumBanner(e.target.files?.[0])}/></label>}</div></>}
  ...
  return <main className="shell">
    ...
    <aside className="rightCol commerceOnlyCol"><div className="rankCard commerceSpotlight"><div className="rankTitle"><h3>🏪 COMÉRCIOS EM DESTAQUE</h3><button onClick={()=>setActive('Comércios')}>Ver todos →</button></div><p className="rightCommerceSubtitle">Destaques somente de {cityLabel}.</p>{topBusinesses.length?topBusinesses.slice(0,5).map((b,i)=><button className="rankRow commerceRankRow" key={b.id} onClick={()=>loadBusiness(b.id)}><b className={`medal m${i+1}`}>{i+1}</b><Avatar profile={{avatar_url:b.logo_url}} name={b.name}/><div><b>{b.name}</b><small>{b.category||'Estabelecimento local'}</small></div><strong>★ {Number(b.rating_average||0).toFixed(1)}</strong></button>):<div className="rankEmpty">Nenhum estabelecimento em destaque ainda.</div>}</div><div className="rankCard localAdCarousel"><div className="rankTitle"><h3>📣 PROPAGANDAS DA CIDADE</h3><small>Comércio local</small></div><div className="adCarouselTrack">{topBusinesses.filter(b=>b.cover_url||b.logo_url).slice(0,8).map(b=><button className="localAdCard" key={`ad-${b.id}`} onClick={()=>loadBusiness(b.id)} style={b.cover_url?{backgroundImage:`linear-gradient(180deg,rgba(3,7,12,.05),rgba(3,7,12,.86)),url(${b.cover_url})`}:{}}><span>DESTAQUE LOCAL</span><b>{b.name}</b><small>{b.category||'Estabelecimento'} • ★ {Number(b.rating_average||0).toFixed(1)}</small></button>)}{!topBusinesses.some(b=>b.cover_url||b.logo_url)&&<div className="rankEmpty">As propagandas locais aparecerão aqui.</div>}</div></div><button className="supportLocal" onClick={()=>{setSelectedBusiness(null);setActive('Comércios')}}>🛍️ <div><b>APOIE O COMÉRCIO LOCAL</b><small>Descubra negócios de {profile?.city||'sua cidade'}</small></div><span>→</span></button><div className="rightContact"><b>CONTATO CIDARANK</b><a href="mailto:cidarankk@gmail.com">✉ cidarankk@gmail.com</a><a href="https://instagram.com/cidarank" target="_blank" rel="noreferrer">📸 @cidarank</a></div></aside>
  </main>
}
