...
export default function Home(){
  ...
  async function hydratePosts(rawPosts){
    ...
    if(ids.length && session?.user?.id){
      ...
      if(caIds.length){
        const {data}=await supabase.from('profiles').select('id,full_name,username,avatar_url,verified,role,account_type').in('id',caIds);
  ...
  function Seal({type='user'}){
    ...
    return <span className={`roleSeal ${type}`} title={type==='verified'?'Perfil verificado pelo CIDARANK':label}>{icon}<span>{label}</span></span>;
  }
  ...
  function RankingCityScreen(){
    ...
    return <><PageHeader title="Ranking da Cidade" subtitle={`Acompanhe sua posição e os destaques de ${cityLabel}.`}/>
      <div className="panel rankingChooser"><div className="segmented wrap">
        <button className={popularView==='city'?'active':''} onClick={()=>setPopularView('city')}>Mais popular da cidade</button><button className={popularView==='week'?'active':''} onClick={()=>setPopularView('week')}>Ranking semanal</button><button className={popularView==='month'?'active':''} onClick={()=>setPopularView('month')}>Ranking mensal</button><button className={popularView==='year'?'active':''} onClick={()=>setPopularView('year')}>Ranking anual</button><button disabled>Ranking do Estado • EM BREVE</button><button disabled>Ranking Brasil • EM BREVE</button><button className={popularView==='business'?'active':''} onClick={()=>setPopularView('business')}>Estabelecimentos em destaque</button>
      </div></div>
