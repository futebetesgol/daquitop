...
export default function Home(){
  ...
  async function hydratePosts(rawPosts){
    ...
    if(ids.length && session?.user?.id){
      ...
      if(caIds.length){
        const {data}=await supabase.from('profiles').select('id,full_name,username,avatar_url,verified,role,account_type,seal_color').in('id',caIds);
  ...
  function Seal({type='user',color='blue'}){
    ...
    return <span className={`roleSeal ${type}`} style={type==='verified'?sealColorStyle(color):undefined} title={type==='verified'?'Perfil verificado pelo CIDARANK':label}>{icon}<span>{label}</span></span>;
  }
  ...
  function PostCard({post,authors=postAuthors,bizMap=postBusinesses,interactive=true}){
    ...
    const renderComment=(c,isReply=false)=>{const ca=commentAuthors[c.author_id]||{};const replies=repliesFor(c.id);return <div className={`commentThread ${isReply?'replyThread':''}`} key={c.id}><div className="comment"><Avatar profile={ca} name={ca.full_name||ca.username} size="xs"/><div className="commentBubble"><div className="commentName"><b>{ca.full_name||ca.username||'Usuário'}</b><Seal type={sealTypeFor(ca)} color={ca.seal_color}/><button className={commentLikes.has(c.id)?'commentLike active':'commentLike'} onClick={()=>toggleCommentLike(c.id)}>♥ <span>{commentLikeCounts[c.id]||0}</span></button></div><p>{c.content}</p><div className="commentMeta"><small>{fmtDate(c.created_at)}</small>{!isReply&&<button onClick={()=>setReplyingTo(v=>({...v,[post.id]:v[post.id]===c.id?null:c.id}))}>Responder</button>}</div></div></div>{replyingTo[post.id]===c.id&&!isReply&&<div className="replyComposer"><span>↳ Respondendo a <b>@{ca.username||'usuario'}</b></span><div><input value={commentReplyDraft[c.id]||''} onChange={e=>setCommentReplyDraft(v=>({...v,[c.id]:e.target.value}))} placeholder={`Responder a @${ca.username||'usuario'}...`}/><button onClick={()=>addComment(post.id,c.id)}>Responder</button></div></div>}{replies.map(r=>renderComment(r,true))}</div>};
    return <article className="post" key={post.id}>
      <div className="postHead"><button className="plainBtn" onClick={()=>author.id&&openProfile(author)}><Avatar profile={business?{avatar_url:business.logo_url}:author} name={name}/></button><div><div className="nameWithSeal"><button className="linkName" onClick={()=>business?loadBusiness(business.id):author.id&&openProfile(author)}>{name}</button><Seal type={business?'business':sealTypeFor(author)}/></div><small>⌖ {post.city} - {post.state} • {fmtDate(post.created_at)} {business&&'• Comércio'}</small></div>{interactive&&post.author_id===session.user.id&&<button className="ghostIcon" onClick={()=>deletePost(post)}>🗑</button>}</div>
      {post.content&&<p>{post.content}</p>}{post.image_url&&<img className="postImage" src={post.image_url} alt="Publicação"/>}
      {interactive&&<><div className="stats"><button className={likedPosts.has(post.id)?'liked':''} onClick={()=>toggleLike(post.id)}>{likedPosts.has(post.id)?'♥':'♡'} {post.likes_count||0} Curtidas</button><span>💬 {post.comments_count||comments.length} Comentários</span></div>
      <div className="daquitop_comments">{roots.map(c=>renderComment(c))}<div className="commentInput"><Avatar profile={profile} size="xs"/><input value={commentDraft[post.id]||''} onChange={e=>setCommentDraft(v=>({...v,[post.id]:e.target.value}))} placeholder="Escreva um comentário..."/><button onClick={()=>addComment(post.id)}>Enviar</button></div></div></>}
    </article>
  }
  ...
  function RankingCityScreen(){
    ...
    const isScope=popularView==='state'||popularView==='brazil';
    const rank=rankType?competitionRanking(rankType):popularView==='city'?popularRank:isScope?scopeRanking:[];
    ...
    const metric=p=>(popularView==='city'||isScope)?`${p.followers_count||0} seguidores`:rankType==='week'?`${p.votes||0} voto(s)`:`${p.points||0} pontos`;
    const title=popularView==='city'?'Mais Popular da Cidade':popularView==='state'?`Ranking do Estado • ${profile.state}`:popularView==='brazil'?'Ranking Brasil':rankType==='week'?'Ranking Semanal':rankType==='month'?'Ranking Mensal':'Ranking Anual';
    const scopeLabel=popularView==='state'?profile.state:popularView==='brazil'?'BRASIL':cityLabel;
    return <><PageHeader title="Ranking" subtitle={`Acompanhe posições locais, estaduais e nacionais do CIDARANK.`}/>
      <div className="panel rankingChooser"><div className="segmented wrap">
        <button className={popularView==='city'?'active':''} onClick={()=>setPopularView('city')}>Popular da cidade</button><button className={popularView==='week'?'active':''} onClick={()=>setPopularView('week')}>Semanal</button><button className={popularView==='month'?'active':''} onClick={()=>setPopularView('month')}>Mensal</button><button className={popularView==='year'?'active':''} onClick={()=>setPopularView('year')}>Anual</button><button className={popularView==='state'?'active':''} onClick={()=>setPopularView('state')}>Estado</button><button className={popularView==='brazil'?'active':''} onClick={()=>setPopularView('brazil')}>Brasil</button><button className={popularView==='business'?'active':''} onClick={()=>setPopularView('business')}>Estabelecimentos</button>
      </div></div>
      <div className="panel rankHow"><h2>Como funciona?</h2><p><b>Semanal:</b> votos da própria cidade. <b>Mensal:</b> pontos das semanas do mês. <b>Anual:</b> pontos dos resultados mensais. <b>Estado e Brasil:</b> classificação geral por seguidores válidos, calculada no servidor para funcionar em qualquer cidade e estado do país.</p><div className="rankFlow"><span>SEMANA</span><b>→</b><span>MÊS</span><b>→</b><span>ANO</span><b>•</b><span>ESTADO</span><b>•</b><span>BRASIL</span></div></div>
      {popularView!=='business'&&<><section className="panel mainRankStage"><div className="panelTitle"><div><span className="eyebrow">AO VIVO • {scopeLabel}</span><h2>{title}</h2><p>TOP 3 em destaque e classificação numerada logo abaixo.</p></div><span className="livePill">● ATUALIZAÇÃO AUTOMÁTICA</span></div>{scopeRankingBusy&&isScope?<div className="rankEmptyState"><span>⏳</span><b>Carregando ranking...</b></div>:top3.length?<div className="floatingPodium rankPagePodium">{order.map(p=>{const pos=rank.findIndex(x=>x.id===p.id)+1;return <button className={`floatingWinner place${pos} animatedRankPerson`} key={p.id} onClick={()=>openProfile(p)}><span className="floatingPlace">#{pos}</span><span className="floatingCrown">{pos===1?'♛':pos===2?'◆':'▲'}</span><span className="rankPhotoSquare"><Avatar profile={p} name={p.full_name} size={pos===1?'lg':'md'}/></span><div className="winnerIdentity"><b>{p.full_name||p.username}</b><Seal type={sealTypeFor(p)} color={p.seal_color}/></div><small>@{p.username}</small><strong>{metric(p)}</strong><span className="podiumBase">{pos}º</span></button>})}</div>:<div className="rankEmptyState"><span>🏆</span><b>Ainda não há classificados</b><small>Assim que houver votos, pontos ou seguidores válidos, as posições aparecem automaticamente.</small></div>}{myPos>=0&&<div className="myRankSpot"><span>📍 SUA POSIÇÃO</span><b>#{myPos+1}</b><small>de {rank.length} classificados • {metric(rank[myPos])}</small></div>}{myPos<0&&!scopeRankingBusy&&<div className="myRankSpot"><span>📍 SUA POSIÇÃO</span><b>—</b><small>Ainda não classificado neste ranking.</small></div>}</section>
  ...
  function OwnerPanelScreen(){
    ...
    return <>
      ...
      <div className="ownerTabs">{[
        ['dashboard','📊 Visão geral'],['tempo','⚡ Tempo real'],['usuarios','👥 Usuários'],['moderacao','🛡️ Moderação'],['comercios','🏪 Estabelecimentos'],['rankings','🏆 Rankings'],['suporte','🎫 Suporte'],['equipe','👑 Equipe'],['comunicados','📢 Comunicados'],['banner','🖼️ Banner'],['auditoria','📜 Auditoria'],['sistema','⚙️ Sistema']
      ...
      {ownerTab==='suporte'&&<div className="panel"><div className="ownerTicketHead"><div><h2>Central de suporte</h2><p className="muted">Atendimento em conversa, com responsável, cidade/estado e histórico.</p></div><select value={ownerSupportStatus} onChange={e=>setOwnerSupportStatus(e.target.value)}><option value="all">Todos os status</option><option value="aberto">Abertos</option><option value="em_analise">Em atendimento</option><option value="resolvido">Resolvidos</option></select></div>{supportStatusTickets.map(t=>{const person=messagePeople[t.user_id]||ownerUsers.find(x=>x.id===t.user_id)||{};const ticketCity=t.requester_city||person.city||'Cidade não informada';const ticketState=t.requester_state||person.state||'UF não informada';const assigned=ownerUsers.find(x=>x.id===t.assigned_to);const msgs=ownerSupportMessages.filter(m=>m.ticket_id===t.id);return <div className="ownerTicket" key={t.id}><div className="ownerTicketHead"><div><b>🎫 {t.subject}</b><small>👤 {person.full_name||person.username||'Usuário'} {person.username?`(@${person.username})`:''} • 📍 {ticketCity} - {ticketState}</small><small>✉ {t.requester_email||'E-mail não registrado'} • {t.category} • {fmtDate(t.created_at)}</small><small>🛡 Responsável: {assigned?(assigned.full_name||assigned.username):'Ainda não atribuído'}</small></div><span className={`status ${t.status}`}>{String(t.status||'aberto').replace('_',' ')}</span></div><div className="supportChat"><div className="supportBubble user"><b>Usuário</b><p>{t.message}</p></div>{msgs.map(m=><div className={`supportBubble ${m.sender_role==='user'?'user':'admin'}`} key={m.id}><b>{m.sender_role==='user'?'Usuário':m.sender_role==='owner'?'Dono CIDARANK':'Administrador'}</b><p>{m.message}</p><small>{fmtDate(m.created_at)}</small></div>)}</div>{t.status!=='resolvido'&&<><textarea rows={3} value={ownerReply[t.id]||''} onChange={e=>setOwnerReply(v=>({...v,[t.id]:e.target.value}))} placeholder="Digite uma mensagem para o usuário..."/><div className="ownerActions">{!t.assigned_to&&<button onClick={()=>ownerAssignTicket(t)}>Assumir atendimento</button>}<button className="primaryBtn" onClick={()=>ownerReplyTicket(t,'em_analise')}>Enviar mensagem</button><button className="safeBtn" onClick={()=>ownerReplyTicket(t,'resolvido')}>Resolver chamado</button></div></>}</div>})}{!supportStatusTickets.length&&<p className="muted">Nenhum chamado neste filtro.</p>}</div>}
