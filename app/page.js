...
export default function Home(){
  ...
  function HomeScreen(){return <>
    ...
    {posts.length?<div className="feed">{posts.slice(0,8).map(p=><PostCard key={p.id} post={p}/>)}</div>:<Empty title="O mural da sua cidade está começando.">Seja a primeira pessoa a publicar no DAQUITOP.</Empty>}
  </>}

  function MuralScreen(){return <><PageHeader title="Mural" subtitle={`Tudo que está acontecendo em ${cityLabel}.`}/><Composer text={postText} setText={setPostText} onPublish={()=>publishPost()}/>{posts.length?<div className="feed">{posts.map(p=><PostCard key={p.id} post={p}/>)}</div>:<Empty title="Ainda não há publicações.">Publique a primeira novidade da cidade.</Empty>}</>}
    ...
    if(selectedBusiness){
      ...
      return <><button className="backBtn" onClick={()=>setSelectedBusiness(null)}>← Voltar para comércios</button>
        ...
        <div className="twoCol"><div>
          ...
          <div className="panel"><h2>Publicações do comércio</h2>{businessPosts.length?<div className="feed">{businessPosts.map(p=><PostCard key={p.id} post={p} authors={businessPostAuthors} bizMap={{[selectedBusiness.id]:selectedBusiness}} interactive={false}/>)}</div>:<p className="muted">Este comércio ainda não publicou nada.</p>}</div>
