...
export default function Home(){
  ...
  function OwnerPanelScreen(){
    ...
    return <>
      ...
      {ownerTab==='suporte'&&<div className="panel"><h2>Central de suporte</h2><p className="muted">Cada chamado mostra quem pediu ajuda, cidade e estado. Os filtros acima permitem acompanhar o Brasil inteiro, um estado ou uma cidade.</p>{monitoredTickets.map(t=>{const person=messagePeople[t.user_id]||ownerUsers.find(x=>x.id===t.user_id)||{};const ticketCity=t.requester_city||person.city||'Cidade não informada';const ticketState=t.requester_state||person.state||'UF não informada';return <div className="ownerTicket" key={t.id}><div className="ownerTicketHead"><div><b>{t.subject}</b><small>👤 {person.full_name||person.username||'Usuário'} {person.username?`(@${person.username})`:''} • 📍 {ticketCity} - {ticketState}</small><small>✉ {t.requester_email||'E-mail protegido/não registrado no chamado'} • {t.category} • {fmtDate(t.created_at)}</small></div><span className={`status ${t.status}`}>{String(t.status||'aberto').replace('_',' ')}</span></div><p>{t.message}</p>{t.image_url&&<img src={t.image_url} alt="Anexo do chamado"/>}<textarea rows={3} value={ownerReply[t.id]??t.admin_response??''} onChange={e=>setOwnerReply(v=>({...v,[t.id]:e.target.value}))} placeholder="Resposta do dono/admin..."/><div className="ownerActions"><button onClick={()=>ownerReplyTicket(t,'em_analise')}>Salvar / Em análise</button><button className="safeBtn" onClick={()=>ownerReplyTicket(t,'resolvido')}>Responder e resolver</button></div></div>})}{!monitoredTickets.length&&<p className="muted">Nenhum chamado recebido neste recorte.</p>}</div>}
