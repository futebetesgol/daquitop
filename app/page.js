...
export default function Home(){
  ...
  async function castScopeVote(person,scope){
    ...
    setNotice(scope==='state'?'Voto estadual registrado. Você pode trocar seu voto enquanto a etapa estiver aberta.':'Voto Brasil registrado. Você pode trocar seu voto enquanto a etapa estiver aberta.');
  }
  ...
  function RulesScreen(){
    return <><PageHeader title="Regras do CIDARANK" subtitle="Entenda de forma simples como funcionam rankings, conquistas, estabelecimentos e a comunidade."/>
      ...
      <div className="rulesGrid">
        ...
        <section className="panel ruleCard"><h3>🗺️ Etapa Estadual</h3><p>Quando o Dono liberar a etapa, entram automaticamente os <b>10 melhores do Ranking Anual de cada cidade</b> daquele estado. Usuários de qualquer cidade do mesmo estado podem votar em qualquer classificado estadual. Um eleitor tem um voto na fase e pode trocar enquanto a votação estiver aberta.</p><b>{scopeCompetitionEnabled('state',profile?.state)?(scopeCompetitionOpen('state',profile?.state)?'AO VIVO':'ENCERRADO'):'EM BREVE'}</b><small>{scopeCompetitionEnabled('state',profile?.state)?`Encerra: ${scopeEndLabel('state')}`:'A liberação é feita pelo Painel do Dono.'}</small></section>
        <section className="panel ruleCard"><h3>🇧🇷 Etapa Brasil</h3><p>Quando liberada, entram automaticamente os <b>10 melhores classificados de cada Estado</b>. Na fase Brasil, usuários de qualquer estado podem votar em qualquer participante nacional classificado.</p><b>{scopeCompetitionEnabled('brazil')?(scopeCompetitionOpen('brazil')?'AO VIVO':'ENCERRADO'):'EM BREVE'}</b><small>{scopeCompetitionEnabled('brazil')?`Encerra: ${scopeEndLabel('brazil')}`:'A liberação é feita pelo Painel do Dono.'}</small></section>
